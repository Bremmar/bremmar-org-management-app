...
function HeadlineComposer({ readOnly, onCreate }: { readOnly: boolean; onCreate: (input: Pick<Headline, 'type' | 'title' | 'detail'>) => Promise<boolean> }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setSaving(true);
    if (await onCreate({ type, title: title.trim(), detail: detail.trim() })) {
      setTitle('');
      setDetail('');
      setType('win');
    }
  ...
  return <form className="headline-composer" onSubmit={submit}><div className="headline-composer-heading"><div><span className="section-kicker">ADD AHEAD OF THE MEETING</span><strong>Put a headline on the agenda</strong></div><small>Members can add a win or concern now; it will be carried into this meeting.</small></div><div className="headline-composer-grid"><label>Type<select value={type} onChange={(event) => setType(event.target.value as Headline['type'])}><option value="win">Win</option><option value="concern">Concern</option></select></label><label>Headline<input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="What changed?" required /></label><label className="headline-composer-detail">Detail<textarea value={detail} onChange={(event) => setDetail(event.target.value)} rows={2} placeholder="Add useful context for the room (optional)." /></label></div><div className="headline-composer-actions"><small>Keep it brief and relevant to customers, employees, or the team’s work.</small><Button type="submit" disabled={saving || !title.trim()}>{saving ? 'Adding…' : 'Add headline ＋'}</Button></div></form>;
}
