...
function App() {
  ...
  const renderView = () => {
    ...
    switch (activeView) {
      ...
      case 'issues':
        return <IssuesView {...common} issues={activeIssues} onStartIssue={startIssue} onSolveIssue={solveIssue} onEditIssue={(issueId) => setModal({ type: 'issue-detail', issueId })} onAuditIssue={(issueId) => openAudit('issue', issueId)} onAdd={() => setModal({ type: 'issue' })} onTransfer={(issueId) => setModal({ type: 'transfer', issueId })} />;
      case 'messages':
