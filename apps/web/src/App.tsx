...
function App() {
  ...
  const flagMetric = (metric: ScorecardMetric, result?: ScorecardResult) => {
    if (activeIssues.some((issue) => issue.title === `Scorecard: ${metric.label}`)) {
      notify('That metric is already on the Issues list.');
      return;
    }
    void refresh(workspaceApi.addIssue({ title: `Scorecard: ${metric.label}`, detail: `${metric.label} is ${result?.actual ?? 'Not entered'} against a target of ${metric.target} ${metric.unit} for the week of ${result?.weekStartDate ?? selectedScorecardWeek}.`, category: 'Scorecard', teamId: activeTeam.id, raisedById: workspace.currentUser.id, horizon: 'short-term' }), 'Metric added to the Issues list for IDS.');
  };
  ...
  const renderView = () => {
    ...
    switch (activeView) {
      ...
      case 'meeting':
        ...
        return <MeetingView {...common} agenda={activeAgenda} rocks={activeRocks} todos={activeTodos} issues={activeIssues} metrics={activeMetrics} scorecardResults={activeScorecardResults} headlines={activeHeadlines} meeting={currentMeeting} section={meetingSection} secondsLeft={secondsLeft} running={meetingRunning} closed={meetingClosed || currentMeeting.status === 'closed'} pendingTransfers={pendingForTeam} pendingSourceTransfers={pendingFromTeam} onSelectSection={setMeetingSection} onToggleRunning={() => setMeetingRunning((running) => !running)} onUpdateRock={updateRockStatus} onUpdateTodo={updateTodoStatus} onStartIssue={startIssue} onOpenIssue={(issueId) => setModal({ type: 'issue-detail', issueId, meetingId: currentMeeting.id })} onSolveIssue={solveIssue} onFlagMetric={flagMetric} onAccept={acceptTransfer} onReject={(id) => setModal({ type: 'reject', transferId: id })} onCancel={cancelTransfer} onClose={closeMeeting} onNavigate={(view) => { if (view === 'scorecard') setScorecardWeekStartDate(meetingWeekStartDate); navigate(view); }} />;
      case 'rocks':
  ...
  return (
    <div className="app-shell">
      ...
      {modal?.type === 'todo-detail' && <TodoEditModal workspace={workspace} todo={workspace.todos.find((todo) => todo.id === modal.todoId)!} onClose={() => setModal(null)} onSubmit={async (input) => { const todo = workspace.todos.find((item) => item.id === modal.todoId); if (todo && await refresh(workspaceApi.updateTodo(todo.id, input, todo.version), 'To-Do details saved.')) setModal(null); }} />}
