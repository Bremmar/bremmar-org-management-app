...
import { richTextToPlainText, sanitizeTodoNotes } from './richText';
import type {
  CompanyOverview,
  AdminSnapshot,
  AuditEntityType,
...
const navLabels: Record<ViewId, string> = {
  overview: 'My week', company: 'Company overview', meeting: 'Live L10', 'meeting-prep': 'Meeting prep', 'meeting-history': 'Past meetings', vto: 'Vision / Traction', rocks: 'Rocks', todos: 'To-Dos', issues: 'Issues', messages: 'Team messages', scorecard: 'Scorecard', admin: 'Administration', profile: 'Profile',
};

type AdminSectionId = 'workspace' | 'members' | 'environment' | 'users';

const adminSectionLabels: Record<AdminSectionId, string> = {
...
type AuditTarget = Pick<Rock | Todo | Issue, 'id' | 'title' | 'createdAt' | 'updatedAt' | 'updatedBy' | 'version'>;

function auditTargetFor(workspace: Workspace, entityType: AuditEntityType, entityId: string): AuditTarget | undefined {
  if (entityType === 'rock') return workspace.rocks.find((rock) => rock.id === entityId);
  if (entityType === 'todo') return workspace.todos.find((todo) => todo.id === entityId);
  return workspace.issues.find((issue) => issue.id === entityId && issue.assignmentState !== 'redirected');
}

function auditEntityLabel(entityType: AuditEntityType) {
  return entityType === 'rock' ? 'Rock' : entityType === 'todo' ? 'To-Do' : 'Issue';
}
...
const canManageMeeting = (workspace: Workspace, teamId: string) => ['OrgAdmin', 'TeamLead'].includes(roleFor(workspace, teamId) ?? '');
...
const isTeamOwner = (workspace: Workspace, teamId: string) => isPlatformAdmin(workspace) || workspace.memberships.some((membership) => membership.teamId === teamId && membership.userId === workspace.currentUser.id && membership.active && (membership.role === 'TeamLead' || membership.role === 'OrgAdmin'));
const canAccessAdministration = (workspace: Workspace) => isPlatformAdmin(workspace) || workspace.memberships.some((membership) => membership.userId === workspace.currentUser.id && membership.active && (membership.role === 'TeamLead' || membership.role === 'OrgAdmin'));
const canManageMeetingSummary = (workspace: Workspace, teamId: string) => canWrite(workspace, teamId);

function adminWorkspaceFor(workspace: Workspace, snapshot: AdminSnapshot | null): Workspace {
  if (!snapshot) return workspace;
  const memberCounts = new Map<string, number>();
  snapshot.memberships.filter((membership) => membership.active).forEach((membership) => memberCounts.set(membership.teamId, (memberCounts.get(membership.teamId) ?? 0) + 1));
  return {
    ...workspace,
...
function statusLabel(status: string) {
  return ({ 'on-track': 'On track', 'off-track': 'Off track', complete: 'Complete', open: 'Open', done: 'Done', 'not-done': 'Not done', 'in-ids': 'In IDS', parked: 'Parked', solved: 'Solved', 'in-progress': 'In progress', upcoming: 'Upcoming', closed: 'Completed', skipped: 'Skipped', missed: 'Missed', overdue: 'Overdue', cancelled: 'Cancelled', segue: 'Segue', scorecard: 'Scorecard', 'rock-review': 'Rock Review', headlines: 'Customer & Employee Headlines', 'todo-review': 'To-Do Review', ids: 'IDS', conclude: 'Conclude' } as Record<string, string>)[status] ?? status;
}

const meetingRatingOptions = Array.from({ length: 20 }, (_, index) => (index + 1) / 2);
...
function notificationTeamId(notification: Workspace['notifications'][number]) {
  return notification.recipientTeamId ?? (notification.type === 'team-message' ? notification.teamId : undefined);
}

export function unreadNotificationsForTeam(notifications: Workspace['notifications'], userId: string, selectedTeamId: string, accessibleTeamIds: readonly string[]) {
  return notifications.filter((notification) => {
    if (notification.readAt) return false;
    const recipientTeamId = notificationTeamId(notification);
    const belongsToUser = notification.recipientUserId === userId
      || Boolean(recipientTeamId && accessibleTeamIds.includes(recipientTeamId));
    if (!belongsToUser) return false;
    return notification.type !== 'team-message' || recipientTeamId === selectedTeamId;
  });
}
...
function App() {
  ...
  const [adminSection, setAdminSection] = useState<AdminSectionId>('workspace');
  const [adminSnapshot, setAdminSnapshot] = useState<AdminSnapshot | null>(null);
  const [adminSnapshotRefresh, setAdminSnapshotRefresh] = useState(0);
  ...
  const unreadNotifications = unreadNotificationsForTeam(workspace.notifications, workspace.currentUser.id, activeTeam.id, accessibleTeams.map((team) => team.id));
  ...
  useEffect(() => {
    if (activeView !== 'admin' || !isPlatformAdmin(workspace)) {
      setAdminSnapshot(null);
      return;
    }
    let active = true;
    void workspaceApi.getAdminSnapshot().then((snapshot) => {
      if (active) setAdminSnapshot(snapshot);
    }).catch(() => {
      if (active) setAdminSnapshot(null);
    });
    return () => { active = false; };
  }, [activeView, workspace.environment, workspace.currentUser.id, workspace.currentUser.platformCapabilities, adminSnapshotRefresh]);
  ...
  useEffect(() => {
    ...
    const activeSection = currentMeeting.activeSection && activeAgenda.some((item) => item.id === currentMeeting.activeSection)
      ? currentMeeting.activeSection
      : firstSection;
    setMeetingSection(currentMeeting.status === 'in-progress' ? activeSection : firstSection);
  }, [activeAgenda, currentMeeting?.id, currentMeeting?.status, currentMeeting?.activeSection]);
  ...
  const openAudit = (entityType: AuditEntityType, entityId: string) => {
    setModal({ type: 'audit', entityType, entityId });
  };
  ...
  const refreshAdmin = () => setAdminSnapshotRefresh((value) => value + 1);

  const refreshAdminWorkspace = async (operation: Promise<Workspace>, successMessage: string) => {
    const saved = await refresh(operation, successMessage);
    if (saved) refreshAdmin();
    return saved;
  };
  ...
  const setRockStatus = (rock: Rock, status: RockStatus) => {
    if (rock.status === status) return;
    void refresh(workspaceApi.updateRockStatus(rock.id, status, rock.version), `${rock.title} marked ${statusLabel(status).toLowerCase()}.`);
  };
  ...
  const reopenIssue = (issue: Issue) => {
    void refresh(workspaceApi.reopenIssue(issue.id, issue.version), 'Issue reopened for another IDS conversation.');
  };
  ...
  const createHeadline = (input: Pick<Headline, 'type' | 'title' | 'detail'>) => {
    if (!currentMeeting) return Promise.resolve(false);
    return refresh(workspaceApi.createHeadline({ ...input, teamId: activeTeam.id, meetingId: currentMeeting.id }), 'Headline added to this meeting.');
  };

  const createHeadlineForMeeting = (meetingId: string, input: Pick<Headline, 'type' | 'title' | 'detail'>) => refresh(workspaceApi.createHeadline({ ...input, teamId: activeTeam.id, meetingId }), 'Headline added to this meeting.');
  ...
  const createScorecardMetric = async (input: Pick<ScorecardMetric, 'teamId' | 'label' | 'target' | 'unit' | 'ownerId'>) => {
    if (await refreshAdminWorkspace(workspaceApi.createScorecardMetric(input), 'Measurable definition added.')) setModal(null);
  };

  const updateScorecardMetric = async (metricId: string, input: Partial<Pick<ScorecardMetric, 'label' | 'target' | 'unit' | 'ownerId'>>) => {
    const metric = adminWorkspaceFor(workspace, adminSnapshot).metrics.find((item) => item.id === metricId);
    if (metric && await refreshAdminWorkspace(workspaceApi.updateScorecardMetric(metricId, input, metric.version), 'Measurable definition updated.')) setModal(null);
  };
  ...
  const saveScorecardResultForPrep = (metricId: string, weekStartDate: string, input: Pick<ScorecardResult, 'actual' | 'status'>) => {
    const result = workspace.scorecardResults.find((item) => item.metricId === metricId && item.weekStartDate === weekStartDate);
  ...
  const selectMeetingOccurrence = (meetingId: string) => {
    ...
    setMeetingClosed(completed);
    setRecapMeetingId(completed ? meetingId : null);
    const firstSection = activeAgenda[0]?.id ?? 'conclude';
    setMeetingSection(selected?.status === 'in-progress' && selected.activeSection && activeAgenda.some((item) => item.id === selected.activeSection) ? selected.activeSection : firstSection);
  };
  ...
  const generateMeetingsForTeam = async (teamId: string) => {
    const team = teamFor(adminSnapshot ? { ...workspace, teams: adminSnapshot.teams, meetings: adminSnapshot.meetings } : workspace, teamId);
    if (!team.id || team.id === emptyTeam.id || !isTeamOwner(workspace, teamId)) return;
    const meetings = adminSnapshot?.meetings ?? workspace.meetings;
    const futureOpenCount = meetings.filter((meeting) => meeting.teamId === teamId && meeting.status === 'upcoming' && Number.isFinite(meetingScheduledAt(meeting)) && meetingScheduledAt(meeting) > Date.now()).length;
    if (!window.confirm(`Generate new L10 meetings for ${team.name}? This will replace ${futureOpenCount} future open occurrence${futureOpenCount === 1 ? '' : 's'} with the next four ${cadenceLabel(team.meetingCadence).toLowerCase()} meetings from the saved cadence. Started, completed, and skipped records are preserved.`)) return;
    if (await refresh(workspaceApi.generateMeetings(teamId), 'L10 meetings regenerated from the saved cadence.')) {
      refreshAdmin();
  ...
  const openWorkspaceAdministration = () => {
    setAdminSection('workspace');
    navigate('admin');
  };
  ...
  const renderView = () => {
    ...
    switch (activeView) {
      ...
      case 'meeting-prep':
        return <MeetingPrepView workspace={workspace} team={activeTeam} readOnly={readOnly} meetings={sortedTeamMeetings} rocks={activeRocks} todos={activeTodos} issues={activeIssues} metrics={activeMetrics} scorecardResults={activeScorecardResults} headlines={activeHeadlines} onNavigate={navigate} canManageAdministration={isTeamOwner(workspace, activeTeam.id)} onOpenAdministration={openWorkspaceAdministration} onSaveScorecardResult={saveScorecardResultForPrep} onUpdateRockStatus={setRockStatus} onUpdateTodo={updateTodoStatus} onOpenTodo={(todoId, meetingReadOnly) => setModal({ type: 'todo-detail', todoId, readOnly: meetingReadOnly })} onOpenIssue={(issueId, meetingReadOnly) => setModal({ type: 'issue-detail', issueId, readOnly: meetingReadOnly })} onAddIssue={() => setModal({ type: 'issue' })} onCreateHeadline={createHeadlineForMeeting} />;
      case 'meeting':
        if (!currentMeeting) {
          return <MeetingSetupView team={activeTeam} readOnly={readOnly} canManageAdministration={isTeamOwner(workspace, activeTeam.id)} onNavigate={navigate} onOpenAdministration={openWorkspaceAdministration} />;
        }
      ...
      case 'issues':
        return <IssuesView {...common} issues={activeIssues} onStartIssue={startIssue} onSolveIssue={solveIssue} onReopenIssue={reopenIssue} onEditIssue={(issueId) => setModal({ type: 'issue-detail', issueId })} onAuditIssue={(issueId) => openAudit('issue', issueId)} onAdd={() => setModal({ type: 'issue' })} onTransfer={(issueId) => setModal({ type: 'transfer', issueId })} />;
      case 'messages':
      ...
      case 'scorecard':
        ...
        return <ScorecardView {...common} metrics={activeMetrics} results={activeScorecardResults} weekStartDate={selectedScorecardWeek} onWeekChange={setScorecardWeekStartDate} onFlagMetric={(metric, result) => { if (result) void createScorecardIssue(metric, result); }} onEditResult={(metricId, weekStartDate) => setModal({ type: 'scorecard-result', metricId, weekStartDate })} />;
      case 'admin':
        return <AdminView workspace={workspace} adminSnapshot={adminSnapshot} activeSection={adminSection} onSectionChange={setAdminSection} environmentAccess={environmentAccess} onToggleEnvironmentAccess={updateEnvironmentAccess} onCreateTeam={() => setModal({ type: 'team' })} onCreateUser={() => setModal({ type: 'user' })} onEditUser={(userId) => setModal({ type: 'edit-user', userId })} onUpdateTeam={(teamId, input, expectedVersion) => void refreshAdminWorkspace(workspaceApi.updateTeam(teamId, input, expectedVersion), 'Team settings updated.')} onEditTeam={(teamId) => setModal({ type: 'edit-team', teamId })} onMembership={(input) => void refreshAdminWorkspace(workspaceApi.upsertMembership(input), 'Workspace member updated.')} onRemoveMembership={(teamId, userId, expectedVersion) => void refreshAdminWorkspace(workspaceApi.removeMembership(teamId, userId, expectedVersion), 'Team member removed.')} onSaveSettings={(settings) => void refreshAdminWorkspace(workspaceApi.updateAgeSettings(settings), 'Issue aging settings updated.')} onGenerateMeetings={generateMeetingsForTeam} onAddMetric={(teamId) => setModal({ type: 'scorecard-metric', teamId })} onEditMetric={(metricId, teamId) => setModal({ type: 'scorecard-metric', metricId, teamId })} />;
      case 'profile':
  ...
  const administrationWorkspace = adminWorkspaceFor(workspace, adminSnapshot);
  const activeAuditTarget = modal?.type === 'audit' ? auditTargetFor(workspace, modal.entityType, modal.entityId) : undefined;
    ...
    <div className="app-shell">
      ...
      <main className="main-content">
        ...
        <div className="page-content">
          ...
          {!hasWorkspaceAccess && activeView !== 'admin' ? <NoWorkspaceAccess /> : readOnly && activeView !== 'company' && activeView !== 'admin' && <div className="read-only-banner"><span>◉</span><strong>Read-only workspace</strong><span>{currentRole === 'Viewer' ? 'Your Viewer membership allows review only.' : 'You can review this team because you are part of Leadership. Editing is limited to your assigned teams.'}</span></div>}
          {(hasWorkspaceAccess || activeView === 'admin') && renderView()}
        </div>
      </main>
      ...
      {modal?.type === 'todo-detail' && <TodoEditModal workspace={workspace} todo={workspace.todos.find((todo) => todo.id === modal.todoId)!} readOnly={readOnly || Boolean(modal.readOnly)} onClose={() => setModal(null)} onAudit={() => openAudit('todo', modal.todoId)} onSubmit={async (input) => { const todo = workspace.todos.find((item) => item.id === modal.todoId); if (todo && await refresh(workspaceApi.updateTodo(todo.id, input, todo.version), 'To-Do details saved.')) setModal(null); }} onChecklistAdd={addTodoChecklistItem} onChecklistUpdate={updateTodoChecklistItem} onChecklistDelete={deleteTodoChecklistItem} />}
      ...
      {modal?.type === 'team' && <TeamModal workspace={administrationWorkspace} onClose={() => setModal(null)} onSubmit={async (input) => { if (await refreshAdminWorkspace(workspaceApi.createTeam(input), 'Team created.')) setModal(null); }} />}
      {modal?.type === 'edit-team' && <TeamEditModal workspace={administrationWorkspace} team={administrationWorkspace.teams.find((team) => team.id === modal.teamId)!} onClose={() => setModal(null)} onSubmit={async (input, expectedVersion) => { if (await refreshAdminWorkspace(workspaceApi.updateTeam(modal.teamId, input, expectedVersion), 'Team settings updated.')) setModal(null); }} />}
      ...
      {modal?.type === 'scorecard-metric' && <ScorecardMetricModal workspace={administrationWorkspace} team={administrationWorkspace.teams.find((team) => team.id === (modal.teamId ?? activeTeam.id)) ?? activeTeam} metric={modal.metricId ? administrationWorkspace.metrics.find((metric) => metric.id === modal.metricId) : undefined} onClose={() => setModal(null)} onSubmit={modal.metricId ? (input) => updateScorecardMetric(modal.metricId!, input) : (input) => createScorecardMetric({ ...input, teamId: modal.teamId ?? activeTeam.id })} />}
      ...
      {modal?.type === 'audit' && activeAuditTarget && <AuditTrailModal workspace={workspace} entityType={modal.entityType} entity={activeAuditTarget} onClose={() => setModal(null)} />}
    </div>
  );
}
...
type ModalState =
  ...
  | { type: 'scorecard-metric'; metricId?: string; teamId?: string }
  | { type: 'scorecard-result'; metricId: string; weekStartDate: string }
  | { type: 'audit'; entityType: AuditEntityType; entityId: string }
  | { type: 'team' }
...
function MeetingSetupView({ team, readOnly, canManageAdministration, onNavigate, onOpenAdministration }: { team: Team; readOnly: boolean; canManageAdministration: boolean; onNavigate: (view: ViewId) => void; onOpenAdministration: () => void }) {
  return <><PageHeader eyebrow={`${team.name.toUpperCase()} · ${cadenceLabel(team.meetingCadence).toUpperCase()} RHYTHM`} title="No L10 meeting yet." description="The team workspace is connected, but this environment does not have a meeting record to open." actions={<><Button variant="secondary" onClick={() => onNavigate('overview')}>Back to My week</Button>{canManageAdministration && team.nodeType === 'operational' && <Button onClick={onOpenAdministration}>Manage cadence</Button>}</>} /><div className="loading-card card-surface"><EmptyState title="The meeting shell still needs to be generated" detail={readOnly ? 'Ask the team owner or Platform Admin to generate the next four meetings from the saved cadence.' : 'Open Workspace Administration to review the saved cadence and regenerate the next four meetings.'} /></div></>;
}
...
function Sidebar({ workspace, team, activeView, onView, open }: { workspace: Workspace; team: Team; activeView: ViewId; onView: (view: ViewId) => void; open: boolean }) {
  const items: Array<{ id: ViewId; label: string; icon: string; group?: string }> = [
    ...
    ...(canAccessAdministration(workspace) ? [{ id: 'admin' as ViewId, label: 'Administration', icon: '⚙', group: 'ORGANISATION' }] : []),
  ];
...
function MeetingOccurrencePanel({ team, occurrences, selectedMeetingId, currentUserId, canManageSchedule, canChangeFacilitator, canManageAdministration, onOpenAdministration, onSelect, onStart, onChangeFacilitator, onSkip, onReschedule }: { team: Team; occurrences: Workspace['meetings']; selectedMeetingId: string; currentUserId: string; canManageSchedule: boolean; canChangeFacilitator: boolean; canManageAdministration: boolean; onOpenAdministration: () => void; onSelect: (meetingId: string) => void; onStart: (meeting: Workspace['meetings'][number]) => void; onChangeFacilitator: (meeting: Workspace['meetings'][number]) => void; onSkip: (meeting: Workspace['meetings'][number]) => void; onReschedule: (meeting: Workspace['meetings'][number]) => void }) {
  ...
  return <section className="meeting-occurrence-panel card-surface"><div className="meeting-occurrence-heading"><div><span className="section-kicker">MEETING OCCURRENCES</span><h2>Choose the L10 to open</h2></div><div className="meeting-occurrence-heading-actions"><span>{futureUpcomingCount} upcoming in the rolling window</span>{canManageAdministration && <Button variant="secondary" onClick={onOpenAdministration}>Manage cadence</Button>}</div></div><div className="meeting-occurrence-list">{listed.map((meeting) => { const reviewStatus = meetingReviewStatus(meeting, team); const open = meeting.status === 'upcoming' || meeting.status === 'in-progress'; const canResume = canResumeMeeting(meeting, currentUserId); return <div className={`meeting-occurrence-row ${selectedMeetingId === meeting.id ? 'meeting-occurrence-selected' : ''}`} key={meeting.id}><button className="meeting-occurrence-main" onClick={() => onSelect(meeting.id)}><span className="occurrence-date"><strong>{formatDate(meeting.scheduledDate ?? '')}</strong><small>{meeting.scheduledTime ?? team.meetingTime}</small></span><span className="occurrence-copy"><strong>{meeting.label}</strong><small>{meeting.startedAt ? `Started ${formatTime(meeting.startedAt)}` : meeting.status === 'skipped' ? 'Not started' : 'Ready to start'}</small></span><StatusPill status={reviewStatus} /></button><div className="meeting-occurrence-actions">{canManageSchedule && open && <><Button variant={meeting.status === 'in-progress' ? 'secondary' : 'primary'} onClick={() => onStart(meeting)} disabled={!canResume} title={!canResume ? 'Another facilitator owns this live meeting.' : undefined}>{meeting.status === 'in-progress' ? 'Resume' : 'Start'}</Button>{meeting.status === 'in-progress' && !canResume && canChangeFacilitator && <Button variant="quiet" onClick={() => onChangeFacilitator(meeting)}>Change facilitator</Button>}</>}{canManageSchedule && meeting.status === 'upcoming' && <><Button variant="quiet" onClick={() => onSkip(meeting)}>Skip</Button><Button variant="quiet" onClick={() => onReschedule(meeting)}>Reschedule</Button></>}</div></div>; })}{listed.length === 0 && <EmptyState title="No open meeting occurrences" detail="Completed meetings are available in Past meetings." />}</div></section>;
}

function manualRecapFor(meeting: Workspace['meetings'][number]) {
  if (meeting.manualRecap !== undefined) return meeting.manualRecap;
  const marker = 'Facilitator notes:';
  const markerIndex = meeting.recap.lastIndexOf(marker);
  if (markerIndex >= 0) return meeting.recap.slice(markerIndex + marker.length).trim();
  return meeting.recap.includes(' L10 recap · ') ? '' : meeting.recap;
}

function MeetingRatingSnapshot({ workspace, meeting }: { workspace: Workspace; meeting: Workspace['meetings'][number] }) {
  const ratings = meeting.attendeeRatings ?? [];
  if (!ratings.length) return null;
  const average = averageMeetingRating(ratings) ?? meeting.lastRating;
  const values = ratings.map((entry) => entry.rating);
  const spread = Math.max(...values) - Math.min(...values);
  return <div className="meeting-rating-snapshot"><div className="meeting-rating-snapshot-heading"><div><span className="section-kicker">INDIVIDUAL RATINGS</span><strong>How the room experienced the meeting</strong></div><span><b>{average.toFixed(2)}</b>/10 average</span></div><div className="meeting-rating-snapshot-list">{ratings.map((entry) => { const attendee = userFor(workspace, entry.attendeeId); return <div className="meeting-rating-snapshot-row" key={entry.attendeeId}><span><Avatar user={attendee} size="sm" /><strong>{attendee.name}</strong></span><div className="meeting-rating-score"><span className="meeting-rating-bar" aria-hidden="true"><span style={{ width: `${Math.max(0, Math.min(10, entry.rating)) * 10}%` }} /></span><b>{entry.rating.toFixed(1)}</b></div></div>; })}</div>{ratings.length > 1 && <small className="meeting-rating-snapshot-note">Rating spread: {spread.toFixed(1)} points from lowest to highest.</small>}</div>;
}

function MeetingAiSummaryPanel({ workspace, meeting, canRetry, onRequestSummary, onCancelSummary }: { workspace: Workspace; meeting: Workspace['meetings'][number]; canRetry: boolean; onRequestSummary: () => Promise<boolean>; onCancelSummary: () => Promise<boolean> }) {
  ...
  const cancel = async () => {
    setRequesting(true);
    try {
      await onCancelSummary();
    } finally {
      setRequesting(false);
    }
  };
  return <section className="meeting-ai-summary"><div className="recap-section-heading"><div><span className="section-kicker">AI RECAP</span><h2>Turn the record into a useful next step.</h2></div>{status && <StatusPill status={status} label={status === 'not-generated' ? 'Not generated' : status === 'generating' ? 'Generating' : status === 'queued' ? 'Queued' : status === 'ready' ? 'Ready' : status === 'cancelled' ? 'Cancelled' : 'Failed'} />}</div><MeetingRatingSnapshot workspace={workspace} meeting={meeting} />{(status === 'queued' || status === 'generating') && <div className="ai-summary-state ai-summary-pending"><span className="ai-summary-orbit">✦</span><div><strong>AI summary is being generated</strong><p>The close-time meeting context has been queued. If it stays here, cancel this attempt and resubmit the recap.</p>{canRetry && <div className="ai-summary-actions"><Button variant="quiet" onClick={() => void cancel()} disabled={requesting}>{requesting ? 'Cancelling…' : 'Cancel generation'}</Button></div>}</div></div>}{status === 'cancelled' && <div className="ai-summary-state ai-summary-cancelled"><strong>AI summary generation was cancelled.</strong><p>{meeting.aiSummaryError ?? 'The previous attempt was cancelled before a recap was returned.'}</p>{canRetry && <Button variant="secondary" onClick={() => void request()} disabled={requesting}>{requesting ? 'Queuing…' : 'Resubmit recap'}</Button>}</div>}{status === 'failed' && <div className="ai-summary-state ai-summary-failed"><strong>AI summary could not be generated.</strong><p>{meeting.aiSummaryError ?? 'The AI worker reported an error. The manual recap remains available.'}</p>{canRetry && <Button onClick={() => void request()} disabled={requesting}>{requesting ? 'Retrying…' : 'Retry summary'}</Button>}</div>}{status === 'not-generated' && <div className="ai-summary-state"><strong>No AI summary has been generated yet.</strong><p>This older meeting can be sent through the summary worker once using its persisted meeting record.</p>{canRetry && <Button variant="secondary" onClick={() => void request()} disabled={requesting}>{requesting ? 'Queuing…' : 'Generate summary'}</Button>}</div>}{status === 'ready' && meeting.aiSummary && <div className="ai-summary-ready"><div className="ai-summary-executive"><span className="section-kicker">EXECUTIVE SUMMARY</span><p>{meeting.aiSummary.executiveSummary}</p></div><div className="ai-summary-grid"><SummaryList title="Decisions" items={meeting.aiSummary.decisions} /><SummaryList title="Commitments" items={meeting.aiSummary.commitments} /><SummaryList title="Risks" items={meeting.aiSummary.risks} /><SummaryList title="Next focus" items={meeting.aiSummary.nextFocus} /></div><small className="ai-summary-generated">Generated {meeting.aiSummaryGeneratedAt ? `${formatDate(meeting.aiSummaryGeneratedAt)} · ${formatTime(meeting.aiSummaryGeneratedAt)}` : 'from the meeting record'}.</small>{canRetry && <div className="ai-summary-actions"><Button variant="secondary" onClick={() => void request()} disabled={requesting}>{requesting ? 'Queuing…' : 'Regenerate recap'}</Button></div>}</div>}</section>;
}
...
function recordedRecapSectionsFor(meeting: Workspace['meetings'][number], team: Team) {
  if (!meeting.recap.includes(' L10 recap · ')) return [];
  const configuredLabels = new Set(meetingSectionsFor(team).map((section) => section.label));
  const generatedLabels = new Set(['Scorecard', 'Rock Review', 'Headlines', 'To-Do Review', 'IDS', 'Actions', 'Created To-Dos', 'Meeting IDS notes']);
  const ignoredLabels = new Set(['Facilitator', 'Attendees', 'Meeting duration', 'Meeting rating']);
  const sections: Array<{ heading: string; body: string }> = [];
  for (const line of meeting.recap.split(/\r?\n/)) {
    const separator = line.indexOf(':');
    if (separator <= 0) continue;
    const heading = line.slice(0, separator).trim();
    if (heading === 'Facilitator notes') break;
    if (ignoredLabels.has(heading) || (!configuredLabels.has(heading) && !generatedLabels.has(heading))) continue;
...
function MeetingManualRecapCard({ workspace, team, meeting, variant = 'recap' }: { workspace: Workspace; team: Team; meeting: Workspace['meetings'][number]; variant?: 'recap' | 'history' }) {
  const manualRecap = manualRecapFor(meeting);
  const recordedSections = recordedRecapSectionsFor(meeting, team);
  const fallbackNotes = meetingSectionsFor(team)
    .map((section) => ({ heading: section.label, body: richTextToPlainText(meeting.sectionNotes?.[section.id]) }))
    .filter((section) => section.body.trim());
  const sections = recordedSections.length ? recordedSections : fallbackNotes;
  return <details className={`manual-recap-card card-surface ${variant === 'history' ? 'history-manual-recap' : 'recap-manual'}`}><summary className="manual-recap-summary"><div className="manual-recap-summary-copy"><span className="section-kicker">MANUAL RECAP</span><h2>What the team needs to remember</h2><p>{manualRecap ? 'Final recap, agenda notes, and meeting actions.' : 'Open to review the saved meeting record by L10 section.'}</p></div><div className="manual-recap-summary-meta"><StatusPill status="closed" label={`${meeting.lastRating || '—'}/10 average`} /><span className="manual-recap-summary-toggle">View sections <span aria-hidden="true">⌄</span></span></div></summary><div className="manual-recap-body"><section className="manual-recap-section manual-recap-snapshot"><div className="manual-recap-section-heading"><span className="section-kicker">MEETING SNAPSHOT</span><h3>People and outcome</h3></div><div className="manual-recap-snapshot-grid"><div><span>Facilitator</span><strong>{meeting.facilitatorId ? userFor(workspace, meeting.facilitatorId).name : 'Not recorded'}</strong></div><div><span>Attendees</span><strong>{meeting.attendeeIds.map((id) => userFor(workspace, id).name).join(', ') || 'No attendees recorded.'}</strong></div></div><MeetingRatingSnapshot workspace={workspace} meeting={meeting} /></section><section className="manual-recap-section"><div className="manual-recap-section-heading"><span className="section-kicker">FINAL RECAP</span><h3>Facilitator summary</h3></div>{manualRecap ? <RichTextViewer value={manualRecap} className="manual-recap-rich-text" /> : <p className="manual-recap-empty">No written recap was entered.</p>}</section>{sections.length ? sections.map((section, index) => <section className="manual-recap-section" key={`${section.heading}-${index}`}><div className="manual-recap-section-heading"><span className="section-kicker">L10 SECTION</span><h3>{section.heading}</h3></div>{section.body ? <p className="manual-recap-recorded-copy">{section.body}</p> : <p className="manual-recap-empty">Nothing was recorded for this section.</p>}</section>) : <section className="manual-recap-section"><div className="manual-recap-section-heading"><span className="section-kicker">MEETING RECORD</span><h3>Structured notes</h3></div><p className="manual-recap-empty">No additional section notes were captured.</p></section>}</div></details>;
}

function MeetingRecapView({ workspace, team, meeting, canRetry, onRequestSummary, onCancelSummary, onBack }: { workspace: Workspace; team: Team; meeting: Workspace['meetings'][number]; canRetry: boolean; onRequestSummary: () => Promise<boolean>; onCancelSummary: () => Promise<boolean>; onBack: () => void }) {
  return <><PageHeader eyebrow={`${team.name.toUpperCase()} · MEETING RECAP`} title="Meeting complete." description="The manual recap is saved and the structured AI recap will stay with this meeting record." actions={<Button variant="secondary" onClick={onBack}>Back to Live L10</Button>} /><div className="recap-meta-strip card-surface"><div><span>Meeting</span><strong>{meetingDateTimeLabel(meeting)}</strong></div><div><span>Facilitator</span><strong>{meeting.facilitatorId ? userFor(workspace, meeting.facilitatorId).name : 'Not recorded'}</strong></div><div><span>Started</span><strong>{meeting.startedAt ? `${formatDate(meeting.startedAt)} · ${formatTime(meeting.startedAt)}` : 'Not recorded'}</strong></div><div><span>Closed</span><strong>{meeting.closedAt ? `${formatDate(meeting.closedAt)} · ${formatTime(meeting.closedAt)}` : '—'}</strong></div><div><span>Duration</span><strong>{meeting.durationSeconds !== undefined ? formatDuration(meeting.durationSeconds) : 'Not recorded'}</strong></div><div><span>Attendance</span><strong>{meeting.attendeeIds.length} invited</strong></div></div><div className="recap-layout"><MeetingManualRecapCard workspace={workspace} team={team} meeting={meeting} /><MeetingAiSummaryPanel workspace={workspace} meeting={meeting} canRetry={canRetry} onRequestSummary={onRequestSummary} onCancelSummary={onCancelSummary} /></div><MeetingRecordSections workspace={workspace} meeting={meeting} /></>;
}
...
interface MeetingPrepViewProps {
  ...
  onSaveScorecardResult: (metricId: string, weekStartDate: string, input: Pick<ScorecardResult, 'actual' | 'status'>) => Promise<boolean>;
  onUpdateRockStatus: (rock: Rock, status: RockStatus) => void;
  ...
  onAddIssue: () => void;
  onCreateHeadline: (meetingId: string, input: Pick<Headline, 'type' | 'title' | 'detail'>) => Promise<boolean>;
}

function MeetingPrepView({ workspace, team, readOnly, meetings, rocks, todos, issues, metrics, scorecardResults, headlines, onNavigate, canManageAdministration, onOpenAdministration, onSaveScorecardResult, onUpdateRockStatus, onUpdateTodo, onOpenTodo, onOpenIssue, onAddIssue, onCreateHeadline }: MeetingPrepViewProps) {
  const upcomingMeetings = useMemo(() => meetings
    .filter((meeting) => meeting.status === 'upcoming' && meetingReviewStatus(meeting, team) === 'upcoming')
    .sort((left, right) => meetingScheduledAt(left) - meetingScheduledAt(right)), [meetings, team]);
  const [selectedMeetingId, setSelectedMeetingId] = useState<string | null>(() => upcomingMeetings[0]?.id ?? null);
  const [section, setSection] = useState<MeetingPrepSection>('scorecard');
  const selectedMeeting = upcomingMeetings.find((meeting) => meeting.id === selectedMeetingId) ?? upcomingMeetings[0];
  const weekStartDate = selectedMeeting?.weekStartDate ?? weekStartDateFor(selectedMeeting?.scheduledDate ?? new Date());
  const ownedMetrics = metrics.filter((metric) => metric.ownerId === workspace.currentUser.id);
  const ownedRocks = rocks.filter((rock) => rock.ownerId === workspace.currentUser.id);
  const ownedTodos = todos.filter((todo) => todo.ownerId === workspace.currentUser.id);
  const ownedIssues = issues.filter((issue) => issue.ownerId === workspace.currentUser.id && issue.status !== 'solved');
  const meetingHeadlines = selectedMeeting ? headlines.filter((headline) => !headline.meetingId || headline.meetingId === selectedMeeting.id) : [];
  const sectionIndex = Math.max(0, meetingPrepSections.findIndex((item) => item.id === section));
  const currentPrepSection = meetingPrepSections[sectionIndex] ?? meetingPrepSections[0];
  const setSelectedMeeting = (meetingId: string) => {
    setSelectedMeetingId(meetingId);
    setSection('scorecard');
  };

  useEffect(() => {
    if (selectedMeetingId && upcomingMeetings.some((meeting) => meeting.id === selectedMeetingId)) return;
    setSelectedMeetingId(upcomingMeetings[0]?.id ?? null);
  }, [selectedMeetingId, upcomingMeetings]);

  if (!selectedMeeting) {
    return <><PageHeader eyebrow={`${team.name.toUpperCase()} · MEETING PREP`} title="No upcoming L10 to prepare for." description="When the next meeting occurrence is scheduled, this page will collect the work you own before the room opens." actions={<><Button variant="secondary" onClick={() => onNavigate('meeting')}>Open Live L10 →</Button>{canManageAdministration && <Button variant="secondary" onClick={onOpenAdministration}>Manage cadence</Button>}</>} /><div className="meeting-prep-empty card-surface"><EmptyState title="Your meeting queue is clear" detail="There are no future L10 occurrences in the current rolling window. Team owners can regenerate the queue from Workspace Administration." /></div></>;
  }

  const completedScorecards = ownedMetrics.filter((metric) => scorecardResults.some((result) => result.metricId === metric.id && result.weekStartDate === weekStartDate)).length;
  const completedTodos = ownedTodos.filter((todo) => todo.status === 'done').length;
  const sectionCounts: Record<MeetingPrepSection, string> = {
    scorecard: `${completedScorecards}/${ownedMetrics.length}`,
    rocks: ownedRocks.length ? `${ownedRocks.filter((rock) => rock.status === 'off-track').length} off track` : 'None owned',
    headlines: `${meetingHeadlines.length} captured`,
    todos: `${completedTodos}/${ownedTodos.length} done`,
    issues: `${ownedIssues.length} open`,
  };
  const moveSection = (direction: -1 | 1) => {
    const next = meetingPrepSections[Math.min(meetingPrepSections.length - 1, Math.max(0, sectionIndex + direction))];
    if (next) setSection(next.id);
  };

  return <>
    <PageHeader eyebrow={`${team.name.toUpperCase()} · MEETING PREP`} title="Come ready to the room." description="Prepare the work you own before the L10 starts. Updates are saved to the same team records the meeting will use." actions={<><Button variant="secondary" onClick={() => onNavigate('meeting')}>Open Live L10 →</Button>{canManageAdministration && <Button variant="secondary" onClick={onOpenAdministration}>Manage cadence</Button>}</>} />
    <section className="meeting-prep-selector card-surface"><div><span className="section-kicker">UPCOMING MEETING</span><strong>{selectedMeeting.label}</strong><span>{meetingDateTimeLabel(selectedMeeting)} · {selectedMeeting.attendeeIds.length} invited</span></div><label>Select meeting<select value={selectedMeeting.id} onChange={(event) => setSelectedMeeting(event.target.value)}>{upcomingMeetings.map((meeting) => <option key={meeting.id} value={meeting.id}>{meeting.label} · {meeting.dateLabel}</option>)}</select></label></section>
    <div className="meeting-prep-layout">
      <aside className="meeting-prep-rail card-surface"><div className="meeting-prep-rail-head"><span className="section-kicker">PREP CHECKLIST</span><strong>Your owned work</strong><small>Only records assigned to {workspace.currentUser.name.split(' ')[0]} are shown.</small></div><nav className="meeting-prep-nav" aria-label="Meeting prep sections">{meetingPrepSections.map((item) => <button type="button" className={`meeting-prep-nav-item ${item.id === section ? 'meeting-prep-nav-active' : ''}`} key={item.id} onClick={() => setSection(item.id)}><span className="meeting-prep-nav-index">{item.id === section ? '•' : item.kicker.slice(0, 2)}</span><span><strong>{item.label}</strong><small>{sectionCounts[item.id]}</small></span></button>)}</nav><div className="meeting-prep-rail-foot"><span>Meeting date</span><strong>{selectedMeeting.dateLabel}</strong><small>Week of {formatDate(weekStartDate)}</small></div></aside>
      <section className="meeting-prep-stage card-surface">
        <div className="meeting-prep-stage-head"><div><span className="section-kicker">{currentPrepSection.kicker}</span><h2>{section === 'scorecard' ? 'Bring your numbers.' : section === 'rocks' ? 'Know your Rock status.' : section === 'headlines' ? 'Bring the customer and employee context.' : section === 'todos' ? 'Close the loop on commitments.' : 'Make your Issues ready for IDS.'}</h2><p>{section === 'scorecard' ? 'Enter the latest actual and call out anything that is off track before the team reviews the number.' : section === 'rocks' ? 'Check every Rock you own and leave the status the room should see.' : section === 'headlines' ? 'Add a concise win or concern so the team can start with shared context.' : section === 'todos' ? 'Review the commitments assigned to you and update their status or details.' : 'Review the Issues you own, update their context, or add a new Issue for the next conversation.'}</p></div><span className="meeting-prep-stage-count">{sectionCounts[section]}</span></div>
        {section === 'scorecard' && <div className="prep-item-list">{ownedMetrics.map((metric) => <PrepScorecardRow key={metric.id} metric={metric} result={scorecardResults.find((result) => result.metricId === metric.id && result.weekStartDate === weekStartDate)} weekStartDate={weekStartDate} readOnly={readOnly} onSave={onSaveScorecardResult} />)}{ownedMetrics.length === 0 && <EmptyState title="No measurables assigned to you" detail="Ask a TeamLead to assign a Scorecard measurable before the next L10." />}</div>}
        {section === 'rocks' && <div className="prep-item-list">{ownedRocks.map((rock) => <PrepRockRow key={rock.id} workspace={workspace} rock={rock} readOnly={readOnly} onUpdate={onUpdateRockStatus} />)}{ownedRocks.length === 0 && <EmptyState title="No Rocks assigned to you" detail="Your Rock review is clear for this meeting." />}</div>}
        {section === 'headlines' && <><div className="prep-headline-context">{meetingHeadlines.length ? meetingHeadlines.map((headline) => <article className="prep-headline-row" key={headline.id}><div className={`prep-headline-type prep-headline-${headline.type}`}>{headline.type === 'win' ? '↗' : '!'}</div><div><strong>{headline.title}</strong><RichTextViewer value={headline.detail} /><small>{headline.type === 'win' ? 'Win' : 'Concern'} · {userFor(workspace, headline.authorId).name}</small></div></article>) : <EmptyState title="No headlines yet" detail="Add the first customer, employee, or team headline below." />}</div><HeadlineComposer readOnly={readOnly} onCreate={(input) => onCreateHeadline(selectedMeeting.id, input)} /></>}
        {section === 'todos' && <div className="prep-item-list">{ownedTodos.map((todo) => <PrepTodoRow key={todo.id} todo={todo} readOnly={readOnly} onUpdate={onUpdateTodo} onOpen={() => onOpenTodo(todo.id, readOnly)} />)}{ownedTodos.length === 0 && <EmptyState title="No To-Dos assigned to you" detail="You have no visible commitments to review for this L10." />}</div>}
        {section === 'issues' && <div className="prep-item-list">{ownedIssues.map((issue) => <PrepIssueRow key={issue.id} workspace={workspace} issue={issue} readOnly={readOnly} onOpen={() => onOpenIssue(issue.id, readOnly)} />)}{ownedIssues.length === 0 && <EmptyState title="No open Issues owned by you" detail="Add an Issue when there is a problem, decision, or opportunity the team needs to solve." />}<div className="prep-add-issue"><div><strong>Need to capture something new?</strong><span>Add it to the team’s IDS queue with an owner and priority.</span></div><Button variant="secondary" onClick={onAddIssue} disabled={readOnly}>Add Issue ＋</Button></div></div>}
        <div className="meeting-prep-stage-footer"><Button variant="quiet" onClick={() => moveSection(-1)} disabled={sectionIndex === 0}>← Previous</Button><span>{sectionIndex + 1} of {meetingPrepSections.length}</span><Button variant="secondary" onClick={() => moveSection(1)} disabled={sectionIndex === meetingPrepSections.length - 1}>Next section →</Button></div>
      </section>
    </div>
  </>;
}

function PrepScorecardRow({ metric, result, weekStartDate, readOnly, onSave }: { metric: ScorecardMetric; result?: ScorecardResult; weekStartDate: string; readOnly: boolean; onSave: (metricId: string, weekStartDate: string, input: Pick<ScorecardResult, 'actual' | 'status'>) => Promise<boolean> }) {
  const [actual, setActual] = useState(result?.actual ?? '');
  const [status, setStatus] = useState<ScorecardResult['status']>(result?.status ?? 'on-track');
  const [savedActual, setSavedActual] = useState(result?.actual ?? '');
  const [savedStatus, setSavedStatus] = useState<ScorecardResult['status']>(result?.status ?? 'on-track');
  const [saving, setSaving] = useState(false);
  useEffect(() => {
    const nextActual = result?.actual ?? '';
    const nextStatus = result?.status ?? 'on-track';
    setActual(nextActual);
    setStatus(nextStatus);
    setSavedActual(nextActual);
    setSavedStatus(nextStatus);
  }, [metric.id, result?.actual, result?.status, weekStartDate]);
  const save = async (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (readOnly || !actual.trim()) return;
    setSaving(true);
    if (await onSave(metric.id, weekStartDate, { actual: actual.trim(), status })) {
      setActual(actual.trim());
      setSavedActual(actual.trim());
      setSavedStatus(status);
    }
    setSaving(false);
  };
  const dirty = actual !== savedActual || status !== savedStatus;
  return <form className="prep-scorecard-row" onSubmit={save}><div className="prep-item-copy"><span className={`metric-status-dot metric-${result?.status ?? 'missing'}`} /><div><strong>{metric.label}</strong><small>Target {metric.target} {metric.unit}</small></div></div><label>Actual<input value={actual} onChange={(event) => setActual(event.target.value)} placeholder="Enter result" disabled={readOnly} required /></label><label>Status<select value={status} onChange={(event) => setStatus(event.target.value as ScorecardResult['status'])} disabled={readOnly}><option value="on-track">On track</option><option value="off-track">Off track</option></select></label><Button type="submit" variant="secondary" disabled={readOnly || saving || !actual.trim() || !dirty}>{saving ? 'Saving…' : result ? 'Save result' : 'Save result'}</Button></form>;
}

function PrepRockRow({ workspace, rock, readOnly, onUpdate }: { workspace: Workspace; rock: Rock; readOnly: boolean; onUpdate: (rock: Rock, status: RockStatus) => void }) {
  const milestones = rockMilestoneCounts(rock);
  return <article className="prep-rock-row"><div className="prep-item-copy"><span className={`priority-marker priority-${rock.priority}`} /><div><strong>{rock.title}</strong><small><Avatar user={userFor(workspace, rock.ownerId)} size="sm" /> Due {formatDate(rock.dueDate)} · {milestones.completed}/{milestones.total} milestones complete</small></div></div><label>Status<select value={rock.status} onChange={(event) => onUpdate(rock, event.target.value as RockStatus)} disabled={readOnly}><option value="on-track">On track</option><option value="off-track">Off track</option><option value="complete">Complete</option></select></label></article>;
}

function PrepTodoRow({ todo, readOnly, onUpdate, onOpen }: { todo: Todo; readOnly: boolean; onUpdate: (todo: Todo, status?: TodoStatus) => void; onOpen: () => void }) {
  return <article className={`prep-todo-row ${todo.status === 'done' ? 'prep-todo-done' : ''}`}><button type="button" className={`todo-checkbox ${todo.status === 'done' ? 'checked' : ''}`} onClick={() => onUpdate(todo, todo.status === 'done' ? 'open' : 'done')} disabled={readOnly} aria-label={`Mark ${todo.title} ${todo.status === 'done' ? 'open' : 'done'}`}>{todo.status === 'done' ? '✓' : ''}</button><div className="prep-item-copy"><div><strong>{todo.title}</strong><small>Due {formatDate(todo.dueDate)} · {todo.origin}</small></div></div><select value={todo.status} onChange={(event) => onUpdate(todo, event.target.value as TodoStatus)} disabled={readOnly} aria-label={`Status for ${todo.title}`}><option value="open">Open</option><option value="done">Done</option><option value="not-done">Not done</option></select><Button variant="quiet" onClick={onOpen}>Review →</Button></article>;
}

function PrepIssueRow({ workspace, issue, readOnly, onOpen }: { workspace: Workspace; issue: Issue; readOnly: boolean; onOpen: () => void }) {
  return <article className="prep-issue-row"><span className="issue-number">P{issue.priority}</span><div className="prep-item-copy"><strong>{issue.title}</strong>{issue.detail ? <RichTextViewer value={issue.detail} /> : <small>No additional context recorded.</small>}<small><AgePill issue={issue} /> · {statusLabel(issue.status)} · owner {userFor(workspace, issue.ownerId ?? issue.raisedById).name}</small></div><StatusPill status={issueStatusClass(issue)} /><Button variant="quiet" onClick={onOpen}>Update →</Button></article>;
}
...
interface MeetingViewV2Props {
  ...
  onCreateHeadline: (input: Pick<Headline, 'type' | 'title' | 'detail'>) => Promise<boolean>;
...
function MeetingViewV2({ workspace, team, readOnly, agenda, rocks, todos, issues, metrics, scorecardResults, headlines, meeting, occurrences, selectedMeetingId, section, clockNow, running, closed, pendingTransfers, pendingSourceTransfers, pendingMessages, canManageSchedule, canManageAdministration, onOpenAdministration, onSelectMeeting, onStartMeeting, onChangeFacilitator, onSkipMeeting, onRescheduleMeeting, onSelectSection, onTransitionSection, onToggleRunning, onUpdateRock, onUpdateTodo, onOpenIssue, onOpenTodo, onOpenMessage, onMarkMessageRead, onCreateIssueFromMessage, onStartIssue, onParkIssue, onSolveIssue, onCreateIssueFromScorecard, onCreateIssueFromRock, onSaveSectionNote, onSelectMeetingIssues, onSaveIssueNote, onReorderIssues, onCreateHeadline, onAccept, onReject, onCancel, onClose, onEditSchedule, onNavigate }: MeetingViewV2Props) {
  ...
  const canChangeFacilitator = canManageMeeting(workspace, team.id);
  const canRateAttendees = !meetingReadOnly && (meeting.facilitatorId === workspace.currentUser.id || canChangeFacilitator);
  const meetingHeadlines = headlines.filter((headline) => !headline.meetingId || headline.meetingId === meeting.id);
  const completeAttendeeRatings: MeetingAttendeeRating[] = meeting.attendeeIds
    ...
    .filter((entry): entry is MeetingAttendeeRating => isValidMeetingRating(entry.rating));
  const hasAllAttendeeRatings = meeting.attendeeIds.every((attendeeId) => isValidMeetingRating(attendeeRatings[attendeeId]));
  const meetingRating = averageMeetingRating(completeAttendeeRatings) ?? 0;
  useEffect(() => {
  ...
  return <>
    ...
    <MeetingOccurrencePanel team={team} occurrences={occurrences} selectedMeetingId={selectedMeetingId} currentUserId={workspace.currentUser.id} canManageSchedule={canManageSchedule && !recordClosed} canChangeFacilitator={canChangeFacilitator} canManageAdministration={canManageAdministration} onOpenAdministration={onOpenAdministration} onSelect={onSelectMeeting} onStart={onStartMeeting} onChangeFacilitator={onChangeFacilitator} onSkip={onSkipMeeting} onReschedule={onRescheduleMeeting} />
    <div className="meeting-workspace">
      ...
      <section className="meeting-stage card-surface">
        ...
        <div className="meeting-stage-body"><MeetingSectionContentV2 section={section} workspace={workspace} team={team} rocks={rocks} todos={todos} issues={shortIssues} metrics={metrics} scorecardResults={scorecardResults} headlines={meetingHeadlines} meeting={meeting} pendingMessages={pendingMessages} recap={recap} setRecap={setRecap} meetingRating={meetingRating} attendeeRatings={attendeeRatings} setAttendeeRating={(attendeeId, value) => setAttendeeRatings((current) => { const next = { ...current }; if (value === undefined) delete next[attendeeId]; else next[attendeeId] = value; return next; })} canRateAttendees={canRateAttendees} hasAllAttendeeRatings={hasAllAttendeeRatings} idsStage={idsStage} idsCursor={idsCursor} setIdsStage={setIdsStage} setIdsCursor={setIdsCursor} readOnly={meetingReadOnly} onUpdateRock={onUpdateRock} onUpdateTodo={onUpdateTodo} onOpenIssue={onOpenIssue} onOpenTodo={onOpenTodo} onOpenMessage={onOpenMessage} onMarkMessageRead={onMarkMessageRead} onCreateIssueFromMessage={onCreateIssueFromMessage} onStartIssue={onStartIssue} onParkIssue={onParkIssue} onSolveIssue={onSolveIssue} onCreateIssueFromScorecard={onCreateIssueFromScorecard} onCreateIssueFromRock={onCreateIssueFromRock} onSaveSectionNote={onSaveSectionNote} onSelectMeetingIssues={onSelectMeetingIssues} onSaveIssueNote={onSaveIssueNote} onReorderIssues={onReorderIssues} onCreateHeadline={onCreateHeadline} onNavigate={onNavigate} /></div>
        <div className="meeting-stage-footer"><button className="footer-nav-button" onClick={() => { const previous = agenda[Math.max(0, currentIndex - 1)]; if (previous) void changeSection(previous.id); }} disabled={currentIndex === 0}>← Previous</button><div className="footer-progress"><ProgressBar value={(currentIndex + 1) / Math.max(1, agenda.length) * 100} /><span>{currentIndex + 1} of {agenda.length}</span></div>{section === 'conclude' && currentIndex === agenda.length - 1 && !recordClosed ? <Button onClick={() => onClose(recap, meetingRating, completeAttendeeRatings)} disabled={meetingReadOnly || !hasAllAttendeeRatings}>Close meeting ✓</Button> : <button className="footer-nav-button footer-nav-next" onClick={next} disabled={currentIndex === agenda.length - 1}>Next section →</button>}</div>
      </section>
    </div>
  </>;
}
...
function MeetingNotesField({ meeting, section, title, readOnly, onSave }: { meeting: Workspace['meetings'][number]; section: MeetingSection; title: string; readOnly: boolean; onSave: (section: MeetingSection, note: string) => Promise<boolean> }) {
  const persistedNote = sanitizeTodoNotes(meeting.sectionNotes[section] ?? '');
...
function HeadlineComposer({ readOnly, onCreate }: { readOnly: boolean; onCreate: (input: Pick<Headline, 'type' | 'title' | 'detail'>) => Promise<boolean> }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    setSaving(true);
    if (await onCreate({ type, title: title.trim(), detail })) {
      setTitle('');
      setDetail('');
      setType('win');
    }
  ...
  return <form className="headline-composer" onSubmit={submit}><div className="headline-composer-heading"><div><span className="section-kicker">ADD AHEAD OF THE MEETING</span><strong>Put a headline on the agenda</strong></div><small>Members can add a win or concern now; it will be carried into this meeting.</small></div><div className="headline-composer-grid"><label>Type<select value={type} onChange={(event) => setType(event.target.value as Headline['type'])}><option value="win">Win</option><option value="concern">Concern</option></select></label><label>Headline<input value={title} onChange={(event) => setTitle(event.target.value)} placeholder="What changed?" required /></label><label className="headline-composer-detail">Detail<RichTextEditor name="headline-detail" value={detail} disabled={false} placeholder="Add useful context for the room (optional)." onChange={setDetail} /></label></div><div className="headline-composer-actions"><small>Keep it brief and relevant to customers, employees, or the team’s work.</small><Button type="submit" disabled={saving || !title.trim()}>{saving ? 'Adding…' : 'Add headline ＋'}</Button></div></form>;
}
...
interface MeetingSectionContentV2Props {
  ...
  setAttendeeRating: (attendeeId: string, rating: number | undefined) => void;
...
function MeetingSectionContentV2({ section, workspace, team, rocks, todos, issues, metrics, scorecardResults, headlines, meeting, pendingMessages, recap, setRecap, meetingRating, attendeeRatings, setAttendeeRating, canRateAttendees, hasAllAttendeeRatings, idsStage, idsCursor, setIdsStage, setIdsCursor, readOnly, onUpdateRock, onUpdateTodo, onOpenIssue, onOpenTodo, onOpenMessage, onMarkMessageRead, onCreateIssueFromMessage, onStartIssue, onParkIssue, onSolveIssue, onCreateIssueFromScorecard, onCreateIssueFromRock, onSaveSectionNote, onSelectMeetingIssues, onSaveIssueNote, onReorderIssues, onCreateHeadline, onNavigate }: MeetingSectionContentV2Props) {
  ...
  if (section === 'headlines') return <div className="meeting-section-content"><SectionIntro kicker="CUSTOMER & EMPLOYEE HEADLINES · 5 MINUTES" title="Share what changed." description="Wins, concerns, and context help the team see the whole picture before IDS." /><div className="headline-meeting-grid">{headlines.map((headline) => <article className="headline-meeting-card" key={headline.id}><div className="headline-card-label"><span>{headline.type === 'win' ? '↗' : '!'}</span>{headline.type === 'win' ? 'Win' : 'Concern'}<span className="headline-time">{formatDate(headline.createdAt)}</span></div><h3>{headline.title}</h3><RichTextViewer value={headline.detail} /><span className="headline-author"><Avatar user={userFor(workspace, headline.authorId)} size="sm" />{userFor(workspace, headline.authorId).name}</span></article>)}</div>{headlines.length === 0 && <EmptyState title="No headlines have been added yet" detail="Add a win or concern before the meeting so the room can start with shared context." />}<HeadlineComposer readOnly={readOnly} onCreate={onCreateHeadline} /><MeetingNotesField meeting={meeting} section="headlines" title="Customer & Employee headline notes" readOnly={readOnly} onSave={onSaveSectionNote} /></div>;
  ...
  if (section === 'ids') {
    ...
    return <div className="meeting-section-content"><SectionIntro kicker={`IDS · STEP 3 OF 3 · ISSUE ${currentIssueIndex + 1} OF ${orderedIssues.length}`} title={currentIdsIssue.title} description="Identify, discuss, and solve this Issue. Add the room’s thinking below before moving to the next selected Issue." /><div className="ids-work-card"><div className="ids-work-card-head"><div><span className="issue-number">P{currentIdsIssue.priority}</span><StatusPill status={currentIdsIssue.status} /></div><Button variant="quiet" onClick={() => onOpenIssue(currentIdsIssue.id, readOnly)}>Open full Issue</Button></div>{currentIdsIssue.detail ? <RichTextViewer value={currentIdsIssue.detail} className="ids-work-detail" /> : <p className="ids-work-detail">No additional Issue context has been recorded.</p>}<div className="issue-detail-meta"><span><Avatar user={userFor(workspace, currentIdsIssue.ownerId ?? currentIdsIssue.raisedById)} size="sm" /> {userFor(workspace, currentIdsIssue.ownerId ?? currentIdsIssue.raisedById).name}</span><span>{ageLabel(currentIdsIssue)} old · {currentIdsIssue.meetingsPassed} meetings passed</span></div><div className="meeting-notes-panel ids-notes-panel"><div className="meeting-notes-heading"><div><span className="section-kicker">IDS NOTES</span><strong>Identify · Discuss · Solve</strong></div><span className={idsNote.trim() !== savedIdsNote ? 'meeting-notes-status meeting-notes-unsaved' : 'meeting-notes-status'}>{idsNote.trim() !== savedIdsNote ? 'Unsaved changes' : 'Saved to this meeting'}</span></div><RichTextEditor name={`ids-note-${currentIdsIssue.id}`} value={idsNote} onChange={setIdsNote} placeholder="Capture what the team identified, discussed, decided, or needs to remember." disabled={readOnly} /><div className="meeting-notes-actions"><small>Each save is appended to this Issue’s history and the meeting recap.</small><Button variant="secondary" onClick={() => void saveIssueNote()} disabled={readOnly || !idsNote.trim() || idsNote.trim() === savedIdsNote}>Save IDS notes</Button></div></div><div className="ids-work-actions">{currentIdsIssue.status !== 'solved' && <><Button onClick={() => onSolveIssue(currentIdsIssue)} disabled={readOnly}>Solve Issue ✓</Button><Button variant="secondary" onClick={() => onParkIssue(currentIdsIssue)} disabled={readOnly}>Park for next meeting</Button></>}{currentIdsIssue.status === 'solved' && <span className="solved-banner"><span>✓</span><strong>Issue solved</strong></span>}</div></div><div className="ids-stage-actions"><Button variant="quiet" onClick={() => setIdsStage('order')}>← Back to order</Button><div><Button variant="secondary" onClick={() => setIdsCursor(Math.max(0, currentIssueIndex - 1))} disabled={currentIssueIndex === 0}>Previous Issue</Button><Button onClick={() => setIdsCursor(Math.min(orderedIssues.length - 1, currentIssueIndex + 1))} disabled={currentIssueIndex === orderedIssues.length - 1}>Next Issue →</Button></div></div></div>;
  }
  if (section === 'conclude') { const actionSummary = meeting.actionSummary ?? { todosCreated: meeting.createdTodoIds.length, issuesReviewedInIds: meeting.idsIssueIds.length, issuesAddedToIds: meeting.idsAddedIssueIds.length, issuesSolved: meeting.idsIssueIds.filter((issueId) => workspace.issues.find((issue) => issue.id === issueId)?.status === 'solved').length }; const ratingLabel = meetingRating > 0 ? meetingRating.toFixed(2) : '—'; return <div className="meeting-section-content conclude-content"><span className="conclude-symbol">✓</span><span className="section-kicker">CONCLUDE · 5 MINUTES</span><h2>Leave with clarity.</h2><p>Recap the decisions, confirm every To-Do has an owner and due date, and rate the meeting.</p><div className="meeting-action-summary"><div><strong>{actionSummary.todosCreated}</strong><span>To-Dos created</span></div><div><strong>{actionSummary.issuesReviewedInIds}</strong><span>Issues reviewed in IDS</span></div><div><strong>{actionSummary.issuesAddedToIds}</strong><span>Issues added to IDS</span></div><div><strong>{actionSummary.issuesSolved}</strong><span>Issues solved</span></div></div><label className="recap-field">Final recap<textarea value={recap} onChange={(event) => setRecap(event.target.value)} placeholder="What did the team decide?" rows={4} disabled={readOnly} /></label><div className="rating-field"><span className="section-kicker">OVERALL MEETING RATING</span><div className="derived-rating"><strong>{ratingLabel}</strong><span>/ 10 · average of member ratings</span></div><small className="form-help">The meeting rating is calculated automatically from the individual ratings below.</small></div><div className="attendee-rating-field"><div className="attendee-rating-heading"><div><span className="section-kicker">INDIVIDUAL RATINGS</span><strong>How did each person experience the meeting?</strong></div><span>{canRateAttendees ? meeting.facilitatorId === workspace.currentUser.id ? 'Facilitator entry' : 'TeamLead or OrgAdmin entry' : 'Facilitator only'}</span></div><div className="attendee-rating-list">{meeting.attendeeIds.map((attendeeId) => { const attendee = userFor(workspace, attendeeId); return <label className="attendee-rating-row" key={attendeeId}><span><Avatar user={attendee} size="sm" /><strong>{attendee.name}</strong></span><select value={attendeeRatings[attendeeId] ?? ''} onChange={(event) => setAttendeeRating(attendeeId, event.target.value ? Number(event.target.value) : undefined)} disabled={!canRateAttendees}><option value="">Rate</option>{meetingRatingOptions.map((value) => <option value={value} key={value}>{value.toFixed(1)} / 10</option>)}</select></label>; })}{meeting.attendeeIds.length === 0 && <p className="record-empty">No attendees were recorded for this meeting.</p>}</div>{!hasAllAttendeeRatings && !readOnly && <small className="form-help">Choose every attendee in 0.5-point increments from 0.5 to 10 before closing the meeting.</small>}</div></div>; }
...
function RocksView({ workspace, team, readOnly, rocks, onUpdateRock, onEditRock, onAuditRock, onAdd, onAddTask, onOpenTask, onConvertTask, onUpdateTask, onNavigate }: { workspace: Workspace; team: Team; readOnly: boolean; rocks: Rock[]; onUpdateRock: (rock: Rock) => void; onEditRock: (rockId: string) => void; onAuditRock: (rockId: string) => void; onAdd: () => void; onAddTask: (rockId: string) => void; onOpenTask: (taskId: string) => void; onConvertTask: (taskId: string) => void; onUpdateTask: (taskId: string, input: Partial<Pick<RockTask, 'status'>>, expectedVersion?: number) => void; onNavigate: (view: ViewId) => void }) {
...
function TodosView({ workspace, team, readOnly, todos, onUpdateTodo, onEditTodo, onAdd, onNavigate }: { workspace: Workspace; team: Team; readOnly: boolean; todos: Todo[]; onUpdateTodo: (todo: Todo, status?: TodoStatus) => void; onEditTodo: (todoId: string) => void; onAdd: () => void; onNavigate: (view: ViewId) => void }) {
  ...
  return <>
    <PageHeader eyebrow={`${team.name.toUpperCase()} · TEAM COMMITMENTS`} title="Keep the promises visible." description="To-Dos are clear commitments for the next seven days, owned by a person and visible to the team." actions={<><Button variant="secondary" onClick={() => onNavigate('meeting')}>Open L10 agenda</Button>{!readOnly && <Button onClick={onAdd}>Add To-Do ＋</Button>}</>} />
    <div className="todo-summary-grid">
      <button type="button" className={`todo-summary-card card-surface ${filter === 'all' ? 'todo-summary-card-active' : ''}`} onClick={() => setFilter('all')} aria-pressed={filter === 'all'}>
        <span className="summary-icon summary-icon-teal">✓</span>
        <div><strong>{todos.filter((todo) => todo.status === 'done').length}/{todos.length}</strong><span>complete</span></div>
        <ProgressBar value={todos.length ? todos.filter((todo) => todo.status === 'done').length / todos.length * 100 : 0} tone="teal" />
      </button>
      <button type="button" className={`todo-summary-card card-surface ${filter === 'open' ? 'todo-summary-card-active' : ''}`} onClick={() => setFilter('open')} aria-pressed={filter === 'open'}>
        <span className="summary-icon summary-icon-coral">◷</span>
        <div><strong>{todos.filter((todo) => todo.status !== 'done').length}</strong><span>still open</span></div>
        <span className="summary-help">Review before the next L10.</span>
      </button>
      <button type="button" className={`todo-summary-card card-surface ${filter === 'mine' ? 'todo-summary-card-active' : ''}`} onClick={() => setFilter('mine')} aria-pressed={filter === 'mine'}>
        <span className="summary-icon summary-icon-lavender">AK</span>
        <div><strong>{todos.filter((todo) => todo.ownerId === workspace.currentUser.id).length}</strong><span>assigned to you</span></div>
        <span className="summary-help">Your visible commitments.</span>
      </button>
    </div>
    <div className="table-card card-surface">
      <div className="table-card-header"><div><span className="section-kicker">WEEKLY COMMITMENTS</span><h2>Team To-Dos</h2></div><div className="table-filters"><button type="button" className={filter === 'all' ? 'filter-chip filter-chip-active' : 'filter-chip'} onClick={() => setFilter('all')}>All <span>{todos.length}</span></button><button type="button" className={filter === 'open' ? 'filter-chip filter-chip-active' : 'filter-chip'} onClick={() => setFilter('open')}>Open</button><button type="button" className={filter === 'mine' ? 'filter-chip filter-chip-active' : 'filter-chip'} onClick={() => setFilter('mine')}>Mine</button></div></div>
      <div className="todo-table"><div className="table-header"><span /><span>COMMITMENT</span><span>OWNER</span><span>ORIGIN</span><span>DUE</span><span>STATUS</span><span /></div>{visible.map((todo) => <div className={`table-row ${todo.flagged ? 'table-row-flagged' : ''}`} key={todo.id}><button className={`todo-checkbox ${todo.status === 'done' ? 'checked' : ''}`} onClick={() => onUpdateTodo(todo)} disabled={readOnly} aria-label={`Mark ${todo.title} ${todo.status === 'done' ? 'open' : 'done'}`}>{todo.status === 'done' ? '✓' : ''}</button><button className={`table-primary table-primary-button ${todo.status === 'done' ? 'todo-done' : ''}`} onClick={() => onEditTodo(todo.id)}><strong>{todo.title}</strong><small>{todo.linkedRockTaskId ? 'Linked Rock Task' : todo.origin}{todo.flagged ? ` · flagged after ${todo.carryForwardCount} moves` : ''}</small></button><span className="table-person"><Avatar user={userFor(workspace, todo.ownerId)} size="sm" />{userFor(workspace, todo.ownerId).name}</span><span className="table-origin">{todo.origin}</span><span className={`table-due ${todo.status === 'not-done' ? 'due-overdue' : ''}`}>{formatDate(todo.dueDate)}</span><StatusPill status={todo.flagged ? 'not-done' : todo.status} label={todo.flagged ? 'Flagged' : undefined} /></div>)}{visible.length === 0 && <div className="empty-table"><EmptyState title="No To-Dos match" detail="Change the filter or add a new team commitment." /></div>}</div>
    </div>
  </>;
}

function IssuesView({ workspace, team, readOnly, issues, onStartIssue, onSolveIssue, onReopenIssue, onEditIssue, onAuditIssue, onAdd, onTransfer, onNavigate }: { workspace: Workspace; team: Team; readOnly: boolean; issues: Issue[]; onStartIssue: (issue: Issue) => void; onSolveIssue: (issue: Issue) => void; onReopenIssue: (issue: Issue) => void; onEditIssue: (issueId: string) => void; onAuditIssue: (issueId: string) => void; onAdd: () => void; onTransfer: (issueId: string) => void; onNavigate: (view: ViewId) => void }) {
  ...
  const visible = issues.filter((issue) => issue.horizon === horizon && (showSolved || issue.status !== 'solved'));
  const selectedIssue = visible.find((issue) => issue.id === selectedId) ?? visible[0];
  const open = issues.filter((issue) => issue.status !== 'solved');
  const solvedCount = issues.filter((issue) => issue.horizon === horizon && issue.status === 'solved').length;
  ...
  return <>
    <PageHeader eyebrow={`${team.name.toUpperCase()} · STANDALONE ISSUES`} title="Solve the right problem." description="Capture Issues when they appear. The team workspace owns the list; the L10 is one place to solve it." actions={<><Button variant="secondary" onClick={() => onNavigate('meeting')}>Open live IDS</Button>{!readOnly && <Button onClick={onAdd}>Capture Issue ＋</Button>}</>} />
    <div className="issues-stat-row"><div><span className="section-kicker">ACTIVE ISSUES</span><strong>{open.length}</strong><span>visible to the team</span></div><div><span className="section-kicker">3+ MEETINGS</span><strong className="warning-text">{issues.filter((issue) => issue.meetingBand === 'orange' || issue.meetingBand === 'red').length}</strong><span>need attention</span></div><div><span className="section-kicker">UNASSIGNED</span><strong className="negative-text">{unassigned.length}</strong><span>source team triage</span></div><div className="issues-stat-note"><span>⌁</span><p>Short-term Issues feed this week’s IDS. Long-term Issues stay visible for future planning.</p></div></div>
    <div className="issue-horizon-tabs"><button type="button" className={horizon === 'short-term' ? 'horizon-tab-active' : ''} onClick={() => setHorizon('short-term')}>Short-term <span>{issues.filter((issue) => issue.horizon === 'short-term').length}</span></button><button type="button" className={horizon === 'long-term' ? 'horizon-tab-active' : ''} onClick={() => setHorizon('long-term')}>Long-term <span>{issues.filter((issue) => issue.horizon === 'long-term').length}</span></button></div>
    <div className="issues-workbench card-surface"><div className="issues-list-panel"><div className="workbench-panel-head"><div><span className="section-kicker">MEETING HEALTH</span><h2>{horizon === 'short-term' ? 'Weekly Issues' : 'Long-term Issues'}</h2></div><div className="workbench-panel-controls"><span className="sort-label">Priority · meeting count</span><button type="button" className={showSolved ? 'filter-chip filter-chip-active' : 'filter-chip'} onClick={() => setShowSolved((current) => !current)} disabled={!solvedCount}>{showSolved ? 'Hide solved' : 'Show solved'}{solvedCount > 0 && <span>{solvedCount}</span>}</button></div></div>{visible.map((issue) => <button type="button" className={`workbench-issue-row ${selectedIssue?.id === issue.id ? 'workbench-issue-selected' : ''} ${issue.status === 'solved' ? 'workbench-issue-solved' : ''}`} key={issue.id} onClick={() => setSelectedId(issue.id)}><span className="issue-number">{issue.priority}</span><span className="workbench-issue-copy"><strong>{issue.title}</strong><small><AgePill issue={issue} /> · {ageLabel(issue)} old</small></span><StatusPill status={issueStatusClass(issue)} /></button>)}{visible.length === 0 && <EmptyState title={showSolved ? 'No Issues in this horizon' : 'No active Issues in this horizon'} detail={showSolved ? 'Capture the next problem or idea when it appears.' : 'Solved Issues are hidden. Use Show solved to review them.'} />}</div><IssueDetail workspace={workspace} team={team} issue={selectedIssue} readOnly={readOnly} onEditIssue={onEditIssue} onAuditIssue={onAuditIssue} onStartIssue={onStartIssue} onSolveIssue={onSolveIssue} onReopenIssue={onReopenIssue} onTransfer={onTransfer} /></div>
  </>;
}

function IssueDetail({ workspace, team, issue, readOnly, onEditIssue, onAuditIssue, onStartIssue, onSolveIssue, onReopenIssue, onTransfer }: { workspace: Workspace; team: Team; issue?: Issue; readOnly: boolean; onEditIssue: (issueId: string) => void; onAuditIssue: (issueId: string) => void; onStartIssue: (issue: Issue) => void; onSolveIssue: (issue: Issue) => void; onReopenIssue: (issue: Issue) => void; onTransfer: (issueId: string) => void }) {
  if (!issue) {
    return <div className="issue-detail-panel"><EmptyState title="Choose an Issue" detail="Select an item from the list to see its context." /></div>;
  }
  ...
  return (
    <div className="issue-detail-panel">
      <div className="detail-panel-head">
        <StatusPill status={issueStatusClass(issue)} />
        <span className="detail-horizon">{issue.horizon === 'short-term' ? 'Short-term' : 'Long-term'}</span>
        <AgePill issue={issue} />
      </div>
      <div className="detail-title-row">
        <h2>{issue.title}</h2>
        {!readOnly && <Button variant="secondary" onClick={() => onEditIssue(issue.id)}>Edit Details</Button>}
      </div>
      {issue.detail ? <RichTextViewer value={issue.detail} className="issue-detail-copy" /> : <p className="issue-detail-copy">No additional Issue context has been recorded.</p>}
      {issue.linkedRockId && (
        <div className="linked-record">
          <span>Linked Rock</span>
          <strong>{workspace.rocks.find((rock) => rock.id === issue.linkedRockId)?.title ?? 'Quarterly priority'}</strong>
          <span>↗</span>
        </div>
      )}
      <div className="issue-detail-note">
        <span className="section-kicker">HISTORICAL CONTEXT</span>
        {issue.idsNote ? <RichTextViewer value={issue.idsNote} /> : <p>No historical context has been recorded yet.</p>}
      </div>
      {issue.meetingBand === 'red' && (
        <div className="escalation-callout escalation-escalated">
          <strong>Escalated after four L10 meetings</strong>
          <span>{issue.meetingsPassed} meetings passed{issue.escalatedToUserId ? ` · routed to ${userFor(workspace, issue.escalatedToUserId).name}` : ''}</span>
        </div>
      )}
      {issue.assignmentState === 'unassigned' && (
        <div className="unassigned-callout">
          <strong>Unassigned after transfer rejection</strong>
          <span>Choose a new destination team to put this Issue back into motion.</span>
        </div>
      )}
      {transfer && (
        <div className="pending-callout">
          <strong>Transfer pending</strong>
          <span>Waiting for {teamFor(workspace, transfer.destinationTeamId).name}. The source team can cancel this request.</span>
        </div>
      )}
      <div className="issue-detail-meta">
        <span><Avatar user={userFor(workspace, issue.ownerId ?? issue.raisedById)} size="sm" /> {issue.ownerId ? `Owner: ${userFor(workspace, issue.ownerId).name}` : 'Unassigned'}</span>
        <span>Created {formatDate(issue.createdAt)} · {ageLabel(issue)} old</span>
      </div>
      <div className="detail-actions">
        {!isSolved && !readOnly && (
          <>
            <Button variant="secondary" onClick={() => onStartIssue(issue)} disabled={issue.horizon === 'long-term' || issue.assignmentState === 'pending-transfer'}>{issue.status === 'in-ids' ? 'Continue in IDS' : 'Start IDS'} →</Button>
            <Button onClick={() => onSolveIssue(issue)} disabled={issue.assignmentState === 'pending-transfer'}>Mark solved ✓</Button>
            {!transfer && <Button variant="quiet" onClick={() => onTransfer(issue.id)}>Send to another team ⇄</Button>}
          </>
        )}
        {isSolved && !readOnly && <Button variant="secondary" onClick={() => onReopenIssue(issue)}>Reopen for IDS ↺</Button>}
        <Button variant="quiet" onClick={() => onAuditIssue(issue.id)}>Audit trail</Button>
      </div>
      {isSolved && (
        <div className="solved-banner">
          <span>✓</span>
          <div>
            <strong>Solved and removed from the active list</strong>
            <small>The original creation date and decision history remain preserved.</small>
          </div>
        </div>
      )}
      {issueTransfers.length > 0 && (
        <div className="issue-history">
          ...
          {issueTransfers.map((item) => (
            <div className="issue-history-row" key={item.id}>
              <div>
                <strong>{item.status === 'pending' ? 'Transfer pending' : `Transfer ${item.status}`}</strong>
                <small>{teamFor(workspace, item.sourceTeamId).name} → {teamFor(workspace, item.destinationTeamId).name} · {formatDate(item.requestedAt)}</small>
              </div>
              {item.rejectionMessage && <span>{item.rejectionMessage}</span>}
            </div>
          ))}
        </div>
      )}
      <div className="issue-provenance">
        <span>Originated in {sourceTeam.name}</span>
        <span>Version {issue.version}</span>
      </div>
    </div>
  );
}
...
function MessageIssueModal({ workspace, message, onClose, onSubmit }: { workspace: Workspace; message: TeamMessage; onClose: () => void; onSubmit: (input: Pick<Issue, 'title' | 'detail' | 'priority' | 'horizon' | 'ownerId'>) => void }) {
  return <ModalShell title="Create Issue from message" description="The message has prefilled the Issue. Edit the fields before it enters this team’s Issues list." onClose={onClose} className="modal-card-wide"><form className="modal-form" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); onSubmit({ title: String(form.get('title') ?? '').trim(), detail: String(form.get('detail') ?? ''), priority: Number(form.get('priority') ?? 2), horizon: String(form.get('horizon') ?? 'short-term') as IssueHorizon, ownerId: String(form.get('ownerId') ?? workspace.currentUser.id) }); }}><label>Issue title<input name="title" defaultValue={message.subject} autoFocus required /></label><label>Original Context<RichTextEditor name="detail" value={message.body} disabled={false} placeholder="Add enough context for the team to recognise the Issue." /></label><div className="form-grid"><label>Priority<select name="priority" defaultValue="2"><option value="1">1 · highest</option><option value="2">2</option><option value="3">3</option><option value="4">4</option><option value="5">5 · lowest</option></select></label><label>Horizon<select name="horizon" defaultValue="short-term"><option value="short-term">Short-term · feed L10</option><option value="long-term">Long-term · future planning</option></select></label></div><label>Owner<UserOptions workspace={workspace} name="ownerId" /></label><ModalActions onClose={onClose} submitLabel="Create editable Issue" /></form></ModalShell>;
}

function ScorecardView({ workspace, team, readOnly, metrics, results, weekStartDate, onWeekChange, onFlagMetric, onEditResult, onNavigate }: { workspace: Workspace; team: Team; readOnly: boolean; metrics: ScorecardMetric[]; results: ScorecardResult[]; weekStartDate: string; onWeekChange: (weekStartDate: string) => void; onFlagMetric: (metric: ScorecardMetric, result?: ScorecardResult) => void; onEditResult: (metricId: string, weekStartDate: string) => void; onNavigate: (view: ViewId) => void }) {
  const weeks = useMemo(() => {
    const output: string[] = [];
...
function AdminView({ workspace, adminSnapshot, activeSection, onSectionChange, environmentAccess, onToggleEnvironmentAccess, onCreateTeam, onCreateUser, onEditUser, onUpdateTeam, onEditTeam, onMembership, onRemoveMembership, onSaveSettings, onGenerateMeetings, onAddMetric, onEditMetric }: { workspace: Workspace; adminSnapshot: AdminSnapshot | null; activeSection: AdminSectionId; onSectionChange: (section: AdminSectionId) => void; environmentAccess: EnvironmentAccess[] | null; onToggleEnvironmentAccess: (userId: string, testAllowed: boolean) => void; onCreateTeam: () => void; onCreateUser: () => void; onEditUser: (userId: string) => void; onUpdateTeam: (teamId: string, input: TeamUpdateInput, expectedVersion?: number) => void; onEditTeam: (teamId: string) => void; onMembership: (input: MembershipInput) => void; onRemoveMembership: (teamId: string, userId: string, expectedVersion?: number) => void; onSaveSettings: (settings: IssueAgeSettings) => void; onGenerateMeetings: (teamId: string) => void; onAddMetric: (teamId: string) => void; onEditMetric: (metricId: string, teamId: string) => void }) {
  ...
  const header = {
    workspace: { eyebrow: platform ? 'PLATFORM ADMINISTRATION' : 'TEAM OWNER ADMINISTRATION', title: 'Shape the workspace rhythm.', description: 'Keep each team’s operating model clear: cadence, L10 structure, meeting generation, and the measurable definitions that feed the weekly Scorecard.' },
    members: { eyebrow: platform ? 'WORKSPACE MEMBERS' : 'YOUR TEAM MEMBERS', title: 'Keep each roster current.', description: 'Select a team to add or remove members and update their workspace role. Platform capabilities and Test access stay separate and protected.' },
    environment: { eyebrow: 'TEST ENVIRONMENT ACCESS', title: 'Keep Test deliberately separate.', description: 'Grant access to the separate Test database from Live. Test access never changes Live memberships or workspace roles.' },
    users: { eyebrow: 'USER ADMINISTRATION', title: 'Manage the people behind the workspace.', description: 'Maintain directory profiles and platform-level capabilities. Team membership is managed from the selected team’s member screen.' },
  }[currentSection];

  return <>
    <PageHeader eyebrow={header.eyebrow} title={header.title} description={header.description} actions={currentSection === 'workspace' && platform ? <Button onClick={onCreateTeam}>Add team ＋</Button> : currentSection === 'users' && platform ? <Button onClick={onCreateUser}>{isLocalPocBuild ? 'Add local user' : 'Add Entra user'} ＋</Button> : undefined} />
    <div className="admin-shell">
      <nav className="admin-subnav card-surface" aria-label="Administration sections">
        <span className="section-kicker">ADMINISTRATION</span>
        {visibleSections.map((section) => <button type="button" key={section.id} className={`admin-subnav-item ${currentSection === section.id ? 'admin-subnav-item-active' : ''}`} onClick={() => onSectionChange(section.id)}><span className="admin-subnav-icon">{section.icon}</span><span><strong>{section.label}</strong><small>{section.detail}</small></span><span className="admin-subnav-arrow">→</span></button>)}
        {platform && <div className="admin-subnav-note"><strong>Platform Admin</strong><span>Full control across teams, users, roles, and environments.</span></div>}
        {!platform && <div className="admin-subnav-note"><strong>Team Owner</strong><span>You can manage rosters and meeting generation for teams you own.</span></div>}
      </nav>
      <main className="admin-panel">
        {currentSection === 'workspace' && <WorkspaceAdministrationPanel workspace={administrationWorkspace} platform={platform} teams={manageableTeams} selectedTeamId={selectedTeamId} onSelectTeam={setSelectedTeamId} onCreateTeam={onCreateTeam} onUpdateTeam={onUpdateTeam} onEditTeam={onEditTeam} onGenerateMeetings={onGenerateMeetings} onAddMetric={onAddMetric} onEditMetric={onEditMetric} onSaveSettings={onSaveSettings} />}
        {currentSection === 'members' && <WorkspaceMembersPanel workspace={administrationWorkspace} platform={platform} teams={manageableTeams} selectedTeamId={selectedTeamId} onSelectTeam={setSelectedTeamId} onMembership={onMembership} onRemoveMembership={onRemoveMembership} />}
        {currentSection === 'environment' && <EnvironmentAccessPanel workspace={administrationWorkspace} environmentAccess={environmentAccess} onToggle={onToggleEnvironmentAccess} />}
        {currentSection === 'users' && <UserAdministrationPanel workspace={administrationWorkspace} onEditUser={onEditUser} />}
      </main>
    </div>
    {currentSection === 'workspace' && <AdminActivityPanel workspace={administrationWorkspace} />}
  </>;
}

function WorkspaceAdministrationPanel({ workspace, platform, teams, selectedTeamId, onSelectTeam, onCreateTeam, onUpdateTeam, onEditTeam, onGenerateMeetings, onAddMetric, onEditMetric, onSaveSettings }: { workspace: Workspace; platform: boolean; teams: Team[]; selectedTeamId: string; onSelectTeam: (teamId: string) => void; onCreateTeam: () => void; onUpdateTeam: (teamId: string, input: TeamUpdateInput, expectedVersion?: number) => void; onEditTeam: (teamId: string) => void; onGenerateMeetings: (teamId: string) => void; onAddMetric: (teamId: string) => void; onEditMetric: (metricId: string, teamId: string) => void; onSaveSettings: (settings: IssueAgeSettings) => void }) {
  const selectedTeam = teams.find((team) => team.id === selectedTeamId);
  const selectedMetrics = workspace.metrics.filter((metric) => metric.teamId === selectedTeamId);
  return <div className="admin-workspace-stack">
    {platform && <section className="admin-card-section"><div className="admin-section-heading"><div><span className="section-kicker">TEAM HIERARCHY</span><h2>Operational workspaces</h2></div><Button variant="secondary" onClick={onCreateTeam}>Add team ＋</Button></div><div className="hierarchy-card card-surface">{workspace.teams.filter((team) => !team.parentTeamId && team.active).map((team) => <TeamTree key={team.id} workspace={workspace} team={team} depth={0} onToggleType={(id, nodeType, expectedVersion) => onUpdateTeam(id, { nodeType }, expectedVersion)} onEdit={onEditTeam} />)}</div></section>}
    {!platform && <section className="admin-card-section"><div className="admin-section-heading"><div><span className="section-kicker">YOUR OWNED WORKSPACES</span><h2>Team operating models</h2></div></div><div className="owned-team-grid">{teams.map((team) => <button type="button" className={`owned-team-card ${team.id === selectedTeamId ? 'owned-team-card-active' : ''}`} key={team.id} onClick={() => onSelectTeam(team.id)}><span className="team-mark" style={{ backgroundColor: team.accent }}>{team.initials}</span><span><strong>{team.name}</strong><small>{team.memberCount} members · {cadenceLabel(team.meetingCadence)} {team.meetingDay ? `· ${team.meetingDay}` : ''}</small></span></button>)}</div></section>}
    <section className="admin-card-section card-surface admin-team-control-card"><div className="admin-section-heading"><div><span className="section-kicker">TEAM CONFIGURATION</span><h2>{selectedTeam ? selectedTeam.name : 'Select a team'}</h2></div>{selectedTeam && <span className={`node-type-label node-type-${selectedTeam.nodeType}`}>{selectedTeam.nodeType === 'operational' ? 'Operational team' : 'Grouping team'}</span>}</div>{teams.length === 0 ? <EmptyState title="No teams are available to manage" detail="A Platform Admin can assign you as a Team Owner before you manage a roster." /> : selectedTeam ? <><div className="team-admin-selector"><label>Team<select value={selectedTeamId} onChange={(event) => onSelectTeam(event.target.value)}>{teams.map((team) => <option key={team.id} value={team.id}>{team.name}</option>)}</select></label><div className="team-admin-facts"><span><strong>{selectedTeam.memberCount}</strong> members</span><span><strong>{cadenceLabel(selectedTeam.meetingCadence)}</strong> cadence</span><span><strong>{meetingSectionsFor(selectedTeam).length}</strong> L10 sections</span></div></div><div className="team-admin-actions"><div><strong>Meeting cadence and L10 structure</strong><small>{selectedTeam.meetingDay || 'Day not set'}{selectedTeam.meetingTime ? ` · ${selectedTeam.meetingTime}` : ''} · {meetingSectionsFor(selectedTeam).map((section) => section.label).join(' · ')}</small></div><div className="team-admin-buttons">{platform ? <Button variant="secondary" onClick={() => onEditTeam(selectedTeam.id)}>Configure team settings</Button> : <span className="admin-inline-note">Cadence settings are managed by Platform Admin</span>}<Button onClick={() => onGenerateMeetings(selectedTeam.id)} disabled={selectedTeam.nodeType !== 'operational'}>Regenerate L10 meetings</Button></div></div></> : null}</section>
    <section className="admin-card-section card-surface scorecard-definition-card"><div className="admin-section-heading"><div><span className="section-kicker">SCORECARD DEFINITIONS</span><h2>Workspace measurables</h2></div>{selectedTeam && <Button onClick={() => onAddMetric(selectedTeam.id)}>Define measurable ＋</Button>}</div><p className="admin-section-description">Define the target, unit, and accountable owner here. The Scorecard page is reserved for entering weekly actuals and statuses against these definitions.</p>{!selectedTeam ? <EmptyState title="Select a team first" detail="Choose a team above to view its measurable definitions." /> : selectedMetrics.length === 0 ? <EmptyState title="No measurables defined" detail="Define the first measurable for this team. Weekly Scorecard entry will appear once a definition exists." /> : <div className="definition-list">{selectedMetrics.map((metric) => <div className="definition-row" key={metric.id}><div className="definition-name"><span className="metric-status-dot metric-missing" /><div><strong>{metric.label}</strong><small><Avatar user={userFor(workspace, metric.ownerId)} size="sm" />{userFor(workspace, metric.ownerId).name}</small></div></div><span className="definition-target">Target <strong>{metric.target}</strong> {metric.unit}</span><Button variant="quiet" onClick={() => onEditMetric(metric.id, selectedTeam.id)}>Edit definition</Button></div>)}</div>}</section>
    {platform && <LegacySettingsPanel settings={workspace.settings} onSave={onSaveSettings} />}
    {platform && <div className="admin-callout admin-callout-inline"><span className="callout-symbol">✦</span><span className="section-kicker">SOURCE OF TRUTH</span><h2>One workspace operating model.</h2><p>Team cadence, L10 structure, ownership, and Scorecard definitions are configured here so weekly meeting entry stays focused on execution.</p></div>}
  </div>;
}

function WorkspaceMembersPanel({ workspace, platform, teams, selectedTeamId, onSelectTeam, onMembership, onRemoveMembership }: { workspace: Workspace; platform: boolean; teams: Team[]; selectedTeamId: string; onSelectTeam: (teamId: string) => void; onMembership: (input: MembershipInput) => void; onRemoveMembership: (teamId: string, userId: string, expectedVersion?: number) => void }) {
  const [newUserId, setNewUserId] = useState('');
  const [newRole, setNewRole] = useState<TeamRole>('Member');
  const selectedTeam = teams.find((team) => team.id === selectedTeamId);
  const memberships = workspace.memberships.filter((membership) => membership.teamId === selectedTeamId && membership.active);
  const memberIds = new Set(memberships.map((membership) => membership.userId));
  const availableUsers = workspace.users.filter((user) => user.active && !memberIds.has(user.id));
  const roleOptions: TeamRole[] = platform ? ['OrgAdmin', 'TeamLead', 'Member', 'Viewer'] : ['TeamLead', 'Member', 'Viewer'];
  const memberRows = memberships.map((membership) => ({ membership, user: userFor(workspace, membership.userId) })).sort((left, right) => left.user.name.localeCompare(right.user.name));

  useEffect(() => {
    if (!availableUsers.some((user) => user.id === newUserId)) setNewUserId(availableUsers[0]?.id ?? '');
    if (!roleOptions.includes(newRole)) setNewRole('Member');
  }, [availableUsers, newRole, newUserId, roleOptions]);

  return <div className="admin-members-stack">
    <section className="admin-card-section card-surface"><div className="admin-section-heading"><div><span className="section-kicker">TEAM MEMBERS</span><h2>Choose a team</h2></div>{selectedTeam && <span className="membership-count">{memberships.length} active members</span>}</div>{teams.length === 0 ? <EmptyState title="No owned teams" detail="A Team Owner can only manage members of teams they own." /> : <label className="admin-team-select">Team<select value={selectedTeamId} onChange={(event) => onSelectTeam(event.target.value)}>{teams.map((team) => <option key={team.id} value={team.id}>{team.name}</option>)}</select></label>}</section>
    {selectedTeam && <section className="admin-card-section card-surface"><div className="admin-section-heading"><div><span className="section-kicker">{selectedTeam.name.toUpperCase()} · ROSTER</span><h2>Add, remove, or change workspace roles</h2></div><span className="admin-inline-note">Platform capabilities are managed separately</span></div><div className="member-list">{memberRows.map(({ membership, user }) => <div className="member-row" key={membership.id}><Avatar user={user} size="md" /><div className="member-row-copy"><strong>{user.name}</strong><small>{user.email}</small></div>{user.platformCapabilities.includes('PlatformAdmin') && <span className="platform-badge">Platform Admin</span>}<select value={membership.role} onChange={(event) => onMembership({ teamId: selectedTeam.id, userId: user.id, role: event.target.value as TeamRole })} aria-label={`Workspace role for ${user.name}`}>{roleOptions.map((role) => <option key={role} value={role}>{role === 'OrgAdmin' ? 'Org Admin' : role}</option>)}</select><Button variant="danger" onClick={() => { if (window.confirm(`Remove ${user.name} from ${selectedTeam.name}? They will lose access to this team workspace.`)) onRemoveMembership(selectedTeam.id, user.id, membership.version); }}>Remove</Button></div>)}{memberRows.length === 0 && <EmptyState title="No active members" detail="Add the first member below to give this team a workspace presence." />}</div><div className="add-member-form"><div><span className="section-kicker">ADD MEMBER</span><p>Choose an existing directory user and assign a workspace role.</p></div><div className="inline-form"><select value={newUserId} disabled={availableUsers.length === 0} onChange={(event) => setNewUserId(event.target.value)} aria-label="User to add"><option value="">{availableUsers.length ? 'Select a user' : 'All active users are members'}</option>{availableUsers.map((user) => <option key={user.id} value={user.id}>{user.name} · {user.email}</option>)}</select><select value={newRole} onChange={(event) => setNewRole(event.target.value as TeamRole)} aria-label="New workspace role">{roleOptions.map((role) => <option key={role} value={role}>{role === 'OrgAdmin' ? 'Org Admin' : role}</option>)}</select><Button disabled={!newUserId} onClick={() => { onMembership({ teamId: selectedTeam.id, userId: newUserId, role: newRole }); setNewUserId(''); }}>Add member</Button></div></div></section>}
    <div className="admin-permission-note card-surface"><span className="callout-symbol">♙</span><div><strong>{platform ? 'Platform Admin permissions' : 'Team Owner permissions'}</strong><p>{platform ? 'You can manage every team roster and assign any workspace role. Platform Admin capability and Test access remain explicit controls in their own administration sections.' : 'You can add, remove, and update TeamLead, Member, and Viewer roles for teams you own. You cannot grant Platform Admin capability, Org Admin privileges, or Test access.'}</p></div></div>
  </div>;
}

function EnvironmentAccessPanel({ workspace, environmentAccess, onToggle }: { workspace: Workspace; environmentAccess: EnvironmentAccess[] | null; onToggle: (userId: string, testAllowed: boolean) => void }) {
  return <section className="admin-card-section card-surface environment-access-card environment-access-card-standalone"><div className="admin-section-heading"><div><span className="section-kicker">TEST ENVIRONMENT ACCESS</span><h2>Test workspace allowlist</h2></div><span className="admin-inline-note">Managed from Live</span></div><p>Granting access exposes the separate Test database only. Live memberships and roles are unchanged.</p>{workspace.environment !== 'live' && <div className="read-only-banner"><span>◉</span><strong>Switch to Live to change access.</strong><span>Test access grants are administered from the Live environment.</span></div>}{environmentAccess === null ? <div className="loading-card"><span>Loading access grants…</span></div> : <div className="environment-access-list">{environmentAccess.map((access) => <label className="environment-access-row" key={access.userId}><span><strong>{access.name}</strong><small>{access.email}</small></span><input type="checkbox" checked={access.testAllowed} disabled={workspace.environment !== 'live'} onChange={(event) => onToggle(access.userId, event.target.checked)} /><span>{access.testAllowed ? 'Test enabled' : 'No Test access'}</span></label>)}</div>}</section>;
}

function UserAdministrationPanel({ workspace, onEditUser }: { workspace: Workspace; onEditUser: (userId: string) => void }) {
  const activeUsers = workspace.users.filter((user) => user.active).sort((left, right) => left.name.localeCompare(right.name));
  return <div className="admin-users-stack"><section className="admin-card-section card-surface"><div className="admin-section-heading"><div><span className="section-kicker">{isLocalPocBuild ? 'LOCAL DIRECTORY' : 'ENTRA DIRECTORY'}</span><h2>User profiles and platform access</h2></div></div><p className="admin-section-description">Keep directory profiles here. Team memberships and workspace roles are managed from Manage Workspace Members; Test access is managed in its own section.</p><div className="user-directory user-directory-admin">{activeUsers.map((user) => <div className="user-directory-row" key={user.id}><Avatar user={user} size="md" /><div className="user-directory-copy"><strong>{user.name}</strong><span>{user.email}</span></div>{user.platformCapabilities.includes('PlatformAdmin') && <span className="platform-badge">Platform Admin</span>}<span className="membership-count">{workspace.memberships.filter((membership) => membership.userId === user.id && membership.active).length} teams</span><button type="button" className="row-action" onClick={() => onEditUser(user.id)}>Edit</button></div>)}{activeUsers.length === 0 && <EmptyState title="No active users" detail="Add a directory user to begin assigning workspace membership." />}</div></section><div className="admin-callout admin-callout-inline"><span className="callout-symbol">✦</span><span className="section-kicker">{isLocalPocBuild ? 'LOCAL POC MODE' : 'ENTRA ID DIRECTORY'}</span><h2>{isLocalPocBuild ? 'One app-owned source of truth.' : 'One directory identity, one app profile.'}</h2><p>{isLocalPocBuild ? 'Users are local profiles for testing. Authentication can be connected later without changing memberships or authorization rules.' : 'Users are linked to Entra ID by object ID. Memberships and application roles remain managed in this application.'}</p></div></div>;
}

function LegacySettingsPanel({ settings, onSave }: { settings: IssueAgeSettings; onSave: (settings: IssueAgeSettings) => void }) {
  const [draft, setDraft] = useState(settings);
  useEffect(() => setDraft(settings), [settings]);
  return <section className="settings-card card-surface admin-card-section"><div><span className="section-kicker">LEGACY AGE SETTINGS</span><h3>Compatibility thresholds</h3><p>Retained for older records and reports. Current Issue health and escalation use total meetings.</p></div><div className="settings-fields"><label>Aging after (days)<input type="number" min="1" value={draft.agingDays} onChange={(event) => setDraft({ ...draft, agingDays: Number(event.target.value) })} /></label><label>Stale after (days)<input type="number" min="2" value={draft.staleDays} onChange={(event) => setDraft({ ...draft, staleDays: Number(event.target.value) })} /></label><label>Critical after (days)<input type="number" min="3" value={draft.criticalDays} onChange={(event) => setDraft({ ...draft, criticalDays: Number(event.target.value) })} /></label></div><Button onClick={() => onSave(draft)}>Save compatibility thresholds</Button></section>;
}

function AdminActivityPanel({ workspace }: { workspace: Workspace }) {
  return <section className="admin-card-section admin-activity-section"><div className="admin-section-heading"><div><span className="section-kicker">AUDITABILITY</span><h2>Recent administration activity</h2></div></div><div className="audit-list card-surface">{workspace.activity.slice(0, 8).map((event) => <div className="audit-row" key={event.id}><span className={`audit-icon audit-${event.type}`}>{event.type === 'transfer' ? '⇄' : event.type === 'membership' ? '♙' : '•'}</span><div><strong>{event.action}</strong><span>{event.detail}</span></div><time>{formatDate(event.createdAt)} · {formatTime(event.createdAt)}</time></div>)}{workspace.activity.length === 0 && <EmptyState title="No administration activity yet" detail="Important membership and workspace changes will appear here." />}</div></section>;
}

function TeamTree({ workspace, team, depth, onToggleType, onEdit }: { workspace: Workspace; team: Team; depth: number; onToggleType: (teamId: string, nodeType: TeamNodeType, expectedVersion?: number) => void; onEdit: (teamId: string) => void }) {
  const children = workspace.teams.filter((item) => item.parentTeamId === team.id);
  const enabledSections = meetingSectionsFor(team).length;
  return <div className="team-tree-node"><div className="team-tree-row" style={{ paddingLeft: `${16 + depth * 26}px` }}><span className="tree-branch">{children.length ? '⌄' : '•'}</span><span className="team-mark" style={{ backgroundColor: team.accent }}>{team.initials}</span><div><strong>{team.name}</strong><small>{team.memberCount} members · {team.nodeType} · {cadenceLabel(team.meetingCadence)} · {enabledSections} L10 sections · {team.escalationUserIds.length} escalation levels</small></div><button className="node-type-toggle" onClick={() => onToggleType(team.id, team.nodeType === 'operational' ? 'grouping' : 'operational', team.version)}>{team.nodeType === 'operational' ? 'Operational' : 'Grouping only'}</button><button className="row-action" onClick={() => onEdit(team.id)}>Configure</button></div>{children.map((child) => <TeamTree key={child.id} workspace={workspace} team={child} depth={depth + 1} onToggleType={onToggleType} onEdit={onEdit} />)}</div>;
}
...
function ModalShell({ title, description, children, onClose, className }: { title: string; description: string; children: ReactNode; onClose: () => void; className?: string }) {
  return <div className="modal-backdrop" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose(); }}><div className={`modal-card ${className ?? ''}`} role="dialog" aria-modal="true" aria-labelledby="modal-title"><button className="modal-close" onClick={onClose} aria-label="Close dialog">×</button><span className="modal-symbol">✦</span><h2 id="modal-title">{title}</h2><p>{description}</p>{children}</div></div>;
}

function ModalActions({ onClose, submitLabel, onAudit }: { onClose: () => void; submitLabel: string; onAudit?: () => void }) {
  return <div className="modal-actions">{onAudit && <Button variant="quiet" onClick={onAudit}>Audit trail</Button>}<Button variant="secondary" onClick={onClose}>Cancel</Button><Button type="submit">{submitLabel} →</Button></div>;
}

function AuditTrailModal({ workspace, entityType, entity, onClose }: { workspace: Workspace; entityType: AuditEntityType; entity: AuditTarget; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    setEvents(null);
    setError(null);
    void workspaceApi.getAuditTrail(entityType, entity.id)
      .then((next) => {
        if (active) setEvents(next);
      })
      .catch((cause) => {
        if (active) setError(cause instanceof WorkspaceApiError ? cause.message : 'Audit history could not be loaded.');
      });
    return () => { active = false; };
  }, [entity.id, entityType]);

  const latestEvent = events?.[0];
  const lastUpdatedBy = userFor(workspace, latestEvent?.actorId ?? entity.updatedBy);
  return <ModalShell title={`${label} audit trail`} description={`Review the recorded changes for “${entity.title}”. This history is only shown when you open it.`} onClose={onClose} className="modal-card-wide audit-trail-modal">
    <div className="audit-trail-summary">
      <div className="audit-trail-person"><span>Last updated by</span><div><Avatar user={lastUpdatedBy} size="sm" /><strong>{lastUpdatedBy.initials} · {lastUpdatedBy.name}</strong></div><small>{formatDate(entity.updatedAt)} · {formatTime(entity.updatedAt)}</small></div>
      <div><span>Last known version</span><strong>Version {entity.version}</strong><small>{events?.length ?? 0} recorded {events?.length === 1 ? 'event' : 'events'}</small></div>
      <div><span>Created</span><strong>{formatDate(entity.createdAt)}</strong><small>{formatTime(entity.createdAt)}</small></div>
    </div>
    <div className="audit-trail-heading"><span className="section-kicker">CHANGE HISTORY</span><span>{events ? `${events.length} ${events.length === 1 ? 'entry' : 'entries'}` : 'Loading…'}</span></div>
    {error && <div className="audit-trail-state audit-trail-error">{error}</div>}
    {!error && events === null && <div className="audit-trail-state">Loading audit history…</div>}
    {!error && events && (events.length > 0 ? <div className="audit-trail-list">{events.map((event) => { const actor = userFor(workspace, event.actorId); return <article className="audit-trail-event" key={event.id}><span className="audit-trail-initials" aria-hidden="true">{actor.initials}</span><div className="audit-trail-event-copy"><div className="audit-trail-event-top"><strong>{event.action}</strong><time dateTime={event.createdAt}>{formatDate(event.createdAt)} · {formatTime(event.createdAt)}</time></div><div className="audit-trail-event-actor"><span>{actor.name}</span><span>·</span><span>{actor.initials}</span></div><p>{event.detail}</p></div></article>; })}</div> : <div className="audit-trail-state">No recorded changes yet.</div>)}
...
function RichTextViewer({ value, className }: { value: string; className?: string }) {
  return <div className={`rich-text-viewer ${className ?? ''}`} dangerouslySetInnerHTML={{ __html: sanitizeTodoNotes(value) }} />;
}
...
function TodoChecklist({ workspace, todo, readOnly, onAdd, onUpdate, onDelete }: { workspace: Workspace; todo: Todo; readOnly: boolean; onAdd: (todoId: string, text: string, supporterId?: string) => void; onUpdate: (todoId: string, itemId: string, input: Partial<Pick<TodoChecklistItem, 'text' | 'completed' | 'supporterId'>>) => void; onDelete: (todoId: string, itemId: string) => void }) {
  ...
  const draftSupporter = userFor(workspace, draftSupporterId);
  return <section className="todo-checklist"><div className="todo-checklist-heading"><div><span className="section-kicker">CHECKLIST</span><strong>Support the commitment</strong></div><span>{completed}/{todo.checklist.length || 0}</span></div>{todo.checklist.length > 0 && <div className="todo-checklist-list">{todo.checklist.map((item) => <ChecklistRow key={item.id} workspace={workspace} todo={todo} item={item} teamUsers={teamUsers} readOnly={readOnly} onUpdate={onUpdate} onDelete={onDelete} />)}</div>}{todo.checklist.length === 0 && <p className="todo-checklist-empty">Break this To-Do into small steps and assign a Supporter to each one.</p>}{!readOnly && <div className="todo-checklist-add"><input className="todo-checklist-draft-input" value={draft} onChange={(event) => setDraft(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') { event.preventDefault(); add(); } }} placeholder="Add a checklist step" aria-label="New checklist step" /><div className="todo-checklist-add-actions"><div className="checklist-assignment"><button type="button" className="checklist-assign-button" onClick={() => setAssignmentOpen((open) => !open)} aria-expanded={assignmentOpen} aria-haspopup="listbox"><span>Assign</span><strong>{draftSupporterId === todo.ownerId ? 'Owner' : draftSupporter.name}</strong><span aria-hidden="true">⌄</span></button>{assignmentOpen && <div className="supporter-menu checklist-assignment-menu" role="listbox" aria-label="Assign checklist step"><span className="checklist-assignment-label">Assign supporter</span>{teamUsers.map((user) => <button type="button" role="option" aria-selected={user.id === draftSupporterId} key={user.id} onClick={() => { setDraftSupporterId(user.id); setAssignmentOpen(false); }}>{user.name}{user.id === todo.ownerId ? ' · Owner' : ''}{user.id === draftSupporterId ? ' ✓' : ''}</button>)}</div>}</div><Button variant="secondary" onClick={add} disabled={!draft.trim()}>Add step</Button></div></div>}</section>;
}
...
function RockEditModal({ workspace, rock, onClose, onAudit, onSubmit }: { workspace: Workspace; rock: Rock; onClose: () => void; onAudit: () => void; onSubmit: (input: Partial<Pick<Rock, 'title' | 'description' | 'notes' | 'ownerId' | 'dueDate' | 'priority'>>) => void }) {
  return <ModalShell title="Edit Rock details" description="Improve the outcome description or add notes without changing its quarterly identity." onClose={onClose} className="modal-card-wide"><form className="modal-form" onSubmit={(event) => { event.preventDefault(); const form = new FormData(event.currentTarget); onSubmit({ title: String(form.get('title') ?? '').trim(), description: String(form.get('description') ?? '').trim(), notes: String(form.get('notes') ?? ''), ownerId: String(form.get('ownerId') ?? workspace.currentUser.id), priority: String(form.get('priority') ?? rock.priority) as Rock['priority'], dueDate: String(form.get('dueDate') ?? rock.dueDate) }); }}><label>Rock title<input name="title" defaultValue={rock.title} autoFocus required /></label><label>Description<textarea name="description" defaultValue={rock.description} rows={4} required /></label><label>Notes<RichTextEditor name="notes" value={rock.notes} disabled={false} placeholder="Add the context the team needs after creation." /></label><label>Owner<UserOptions workspace={workspace} name="ownerId" defaultValue={rock.ownerId} /></label><label>Priority<select name="priority" defaultValue={rock.priority}><option value="high">High</option><option value="medium">Medium</option><option value="low">Low</option></select></label><label>Due date<input name="dueDate" type="date" defaultValue={rock.dueDate} required /></label><ModalActions onClose={onClose} onAudit={onAudit} submitLabel="Save Rock" /></form></ModalShell>;
}

function IssueEditModal({ workspace, issue, meetingId, readOnly, onClose, onAudit, onSubmit }: { workspace: Workspace; issue: Issue; meetingId?: string; readOnly: boolean; onClose: () => void; onAudit: () => void; onSubmit: (input: Partial<Pick<Issue, 'title' | 'detail' | 'priority' | 'horizon' | 'ownerId' | 'idsNote'>>, meetingNote?: string) => void }) {
  const submit = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (readOnly) return;
    const form = new FormData(event.currentTarget);
    const ownerId = String(form.get('ownerId') ?? '').trim();
    onSubmit({ title: String(form.get('title') ?? '').trim(), detail: String(form.get('detail') ?? ''), priority: Number(form.get('priority') ?? issue.priority), horizon: String(form.get('horizon') ?? issue.horizon) as IssueHorizon, ownerId: ownerId || undefined, idsNote: String(form.get('idsNote') ?? '').trim() }, meetingId ? String(form.get('meetingNote') ?? '').trim() : undefined);
  };
  return <ModalShell title={meetingId ? 'Open Issue during IDS' : 'Edit Issue details'} description={readOnly ? 'This Issue is shown for reference. Closed or read-only meeting records cannot be edited.' : meetingId ? 'Capture notes for this meeting. They are added to the Issue history and meeting recap.' : 'Keep the Issue context current as the team learns more.'} onClose={onClose} className="modal-card-wide"><form className="modal-form" onSubmit={submit}><fieldset className="modal-fieldset" disabled={readOnly}><label>Issue title<input name="title" defaultValue={issue.title} autoFocus required /></label><label>Original Context{readOnly ? <RichTextViewer value={issue.detail} /> : <RichTextEditor name="detail" value={issue.detail} disabled={false} placeholder="Add enough context for the team to recognise the Issue." />}</label><div className="form-grid"><label>Priority<select name="priority" defaultValue={String(issue.priority)}><option value="1">1 · highest</option><option value="2">2</option><option value="3">3</option><option value="4">4</option><option value="5">5 · lowest</option></select></label><label>Horizon<select name="horizon" defaultValue={issue.horizon}><option value="short-term">Short-term · feed L10</option><option value="long-term">Long-term · future planning</option></select></label></div><label>Owner<UserOptions workspace={workspace} name="ownerId" defaultValue={issue.ownerId ?? issue.raisedById} /></label><label>Historical Context{readOnly ? <RichTextViewer value={issue.idsNote ?? ''} /> : <RichTextEditor name="idsNote" value={issue.idsNote ?? ''} disabled={false} placeholder="Keep the running Issue history visible." />}</label>{meetingId && <label className="meeting-note-field">This meeting’s IDS notes<textarea name="meetingNote" rows={5} placeholder="What did the team identify, discuss, decide, or defer today?" /></label>}</fieldset><div className="modal-actions"><Button variant="quiet" onClick={onAudit}>Audit trail</Button><Button variant="secondary" onClick={onClose}>Close</Button>{!readOnly && <Button type="submit">Save Issue →</Button>}</div></form></ModalShell>;
}
...
function TeamEditModal({ workspace, team, onClose, onSubmit }: { workspace: Workspace; team: Team; onClose: () => void; onSubmit: (input: Partial<Pick<Team, 'name' | 'shortName' | 'description' | 'parentTeamId' | 'nodeType' | 'meetingCadence' | 'meetingDay' | 'meetingTime' | 'accent' | 'initials' | 'meetingSections' | 'escalationUserIds'>>, expectedVersion?: number) => void }) {
  const [sections, setSections] = useState(() => meetingSectionConfigsFor(team));
  const recipients = workspace.users.filter((user) => user.active);
  const moveSection = (sectionId: MeetingSection, direction: -1 | 1) => {
    setSections((current) => {
