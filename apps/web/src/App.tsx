...
export function pendingTeamMessagesFor(messages: TeamMessage[], teamId: string) {
  return messages.filter((message) => message.toTeamId === teamId && message.status !== 'converted');
}
...
function App() {
  ...
  const toggleMeetingRunning = async () => {
    ...
    if (readOnly || !currentMeeting || currentMeeting.status === 'closed') return;
...
function MeetingViewV2({ workspace, team, readOnly, agenda, rocks, todos, issues, metrics, scorecardResults, headlines, meeting, section, secondsLeft, running, closed, pendingTransfers, pendingSourceTransfers, pendingMessages, onSelectSection, onToggleRunning, onUpdateRock, onUpdateTodo, onOpenIssue, onOpenTodo, onOpenMessage, onMarkMessageRead, onCreateIssueFromMessage, onStartIssue, onSolveIssue, onCreateIssueFromScorecard, onCreateIssueFromRock, onSaveSectionNote, onReorderIssues, onAccept, onReject, onCancel, onClose, onEditSchedule, onNavigate }: MeetingViewV2Props) {
  const [recap, setRecap] = useState(meeting.recap);
  const [rating, setRating] = useState(meeting.lastRating || 8);
  const currentIndex = Math.max(0, agenda.findIndex((item) => item.id === section));
...
function PendingMessagesPanel({ workspace, messages, readOnly, onOpen, onMarkRead, onCreateIssue }: { workspace: Workspace; messages: TeamMessage[]; readOnly: boolean; onOpen: (messageId: string) => void; onMarkRead: (messageId: string) => void; onCreateIssue: (messageId: string) => void }) {
  return <section className="pending-messages-panel"><div className="pending-messages-panel-header"><div><span className="section-kicker">TEAM MESSAGES</span><h3>Important context for the start</h3></div><span>{messages.length}</span></div>{messages.length === 0 ? <p className="pending-messages-empty">No incoming team messages are waiting.</p> : <div className="pending-messages-list">{messages.map((message) => <article className="pending-message-row" key={message.id}><div className="pending-message-copy"><strong>{message.subject}</strong><small>{teamName(workspace, message.fromTeamId)} · {formatDate(message.createdAt)} · {message.status === 'unread' ? 'Unread' : 'Read'}</small><p>{message.body}</p></div><div className="pending-message-actions"><button className="row-action row-action-small" onClick={() => onOpen(message.id)}>Open</button>{message.status === 'unread' && <button className="row-action row-action-small" onClick={() => onMarkRead(message.id)}>Mark read</button>}{message.status !== 'converted' && !message.convertedIssueId && !readOnly && <button className="row-action row-action-small" onClick={() => onCreateIssue(message.id)}>Create Issue</button>}</div></article>)}</div>}</section>;
}
