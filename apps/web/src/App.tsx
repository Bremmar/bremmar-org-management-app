...
function App() {
  ...
  const generateMeetings = async () => {
    if (readOnly) return;
    const futureOpenCount = teamMeetings.filter((meeting) => meeting.status === 'upcoming' && Number.isFinite(meetingScheduledAt(meeting)) && meetingScheduledAt(meeting) > Date.now()).length;
    if (!window.confirm(`Generate new L10 meetings for ${activeTeam.name}? This will replace ${futureOpenCount} future open occurrence${futureOpenCount === 1 ? '' : 's'} with the next four ${cadenceLabel(activeTeam.meetingCadence).toLowerCase()} meetings from the saved cadence. Started, completed, and skipped records are preserved.`)) return;
    if (await refresh(workspaceApi.generateMeetings(activeTeam.id), 'L10 meetings regenerated from the saved cadence.')) {
      setMeetingClosed(false);
      setRecapMeetingId(null);
      setSelectedMeetingId(null);
    }
...
function MeetingSetupView({ team, readOnly, onNavigate, onGenerateMeetings }: { team: Team; readOnly: boolean; onNavigate: (view: ViewId) => void; onGenerateMeetings: () => void }) {
  return <><PageHeader eyebrow={`${team.name.toUpperCase()} · ${cadenceLabel(team.meetingCadence).toUpperCase()} RHYTHM`} title="No L10 meeting yet." description="The team workspace is connected, but this environment does not have a meeting record to open." actions={<><Button variant="secondary" onClick={() => onNavigate('overview')}>Back to My week</Button>{!readOnly && team.nodeType === 'operational' && <Button onClick={onGenerateMeetings}>Generate meetings</Button>}</>} /><div className="loading-card card-surface"><EmptyState title="The meeting shell still needs to be generated" detail={readOnly ? 'Ask a direct team editor to generate the next four meetings from the saved cadence.' : 'Generate the next four meetings from this team’s saved cadence settings.'} /></div></>;
}
...
function MeetingPrepView({ workspace, team, readOnly, meetings, rocks, todos, issues, metrics, scorecardResults, headlines, onNavigate, onGenerateMeetings, onSaveScorecardResult, onUpdateRockStatus, onUpdateTodo, onOpenTodo, onOpenIssue, onAddIssue, onCreateHeadline }: MeetingPrepViewProps) {
