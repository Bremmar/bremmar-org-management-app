...
import { defaultMeetingSections, meetingSectionsFor, weekStartDateFor } from './types';
import type {
  CompanyOverview,
  EnvironmentAccess,
  EnvironmentId,
...
function App() {
  ...
  const updateMeetingSchedule = async (meetingId: string, input: { scheduledDate: string; scheduledTime: string }) => {
    ...
    if (!meeting) return;
    if (await refresh(workspaceApi.updateMeetingSchedule(activeTeam.id, meetingId, input, meeting.version), 'Current meeting date and time updated.')) setModal(null);
  };
  ...
  return (
    <div className="app-shell">
      ...
      {modal?.type === 'todo-detail' && <TodoEditModal workspace={workspace} todo={workspace.todos.find((todo) => todo.id === modal.todoId)!} readOnly={readOnly || Boolean(modal.readOnly)} onClose={() => setModal(null)} onSubmit={async (input) => { const todo = workspace.todos.find((item) => item.id === modal.todoId); if (todo && await refresh(workspaceApi.updateTodo(todo.id, input, todo.version), 'To-Do details saved.')) setModal(null); }} />}
...
function AgePill({ issue }: { issue: Issue }) {
  const label = issue.ageBand === 'fresh' ? 'Good' : issue.ageBand === 'aging' ? 'Approaching stale' : issue.ageBand === 'stale' ? 'Stale' : 'Critical';
  return <span className={`age-pill ${ageClass(issue.ageBand)}`}><span>{label}</span><small>{ageLabel(issue)}</small></span>;
}
...
function IssuePreview({ workspace, issue, readOnly = false, onClick }: { workspace: Workspace; issue: Issue; readOnly?: boolean; onClick: () => void }) {
  return <button className="issue-preview-row" onClick={onClick} disabled={readOnly}><span className="issue-number">P{issue.priority}</span><span className="issue-preview-copy"><strong>{issue.title}</strong><span><AgePill issue={issue} /> · {issue.category}</span></span><span className="issue-chevron">›</span></button>;
}
...
function MeetingViewV2({ workspace, team, readOnly, agenda, rocks, todos, issues, metrics, scorecardResults, headlines, meeting, section, secondsLeft, running, closed, pendingTransfers, pendingSourceTransfers, onSelectSection, onToggleRunning, onUpdateRock, onUpdateTodo, onOpenIssue, onOpenTodo, onStartIssue, onSolveIssue, onCreateIssueFromScorecard, onCreateIssueFromRock, onSaveSectionNote, onReorderIssues, onAccept, onReject, onCancel, onClose, onEditSchedule, onNavigate }: MeetingViewV2Props) {
  const [recap, setRecap] = useState(meeting.recap);
  const [rating, setRating] = useState(meeting.lastRating || 8);
  const currentIndex = Math.max(0, agenda.findIndex((item) => item.id === section));
  const meetingReadOnly = readOnly || closed || meeting.status === 'closed';
  const shortIssues = issues.filter((issue) => issue.horizon === 'short-term' && issue.status !== 'solved');
  const minutes = Math.floor(secondsLeft / 60).toString().padStart(2, '0');
  const seconds = (secondsLeft % 60).toString().padStart(2, '0');
  const next = () => {
  ...
  return <>
    <div className="meeting-page-header">
      <div><span className="eyebrow">{team.name.toUpperCase()} · {workspace.quarter.label}</span><h1>{closed ? 'Meeting complete.' : 'Run the room.'}</h1><p>{closed ? 'The meeting record is saved. The team can now execute the recap.' : 'Keep the reporting crisp. Put the real work where it belongs: IDS.'}</p></div>
      <div className="meeting-header-actions"><div className={`meeting-status ${closed ? 'meeting-status-closed' : ''}`}><span className="meeting-status-dot" />{closed ? 'Closed' : running ? 'In progress' : 'Ready to start'}</div>{!meetingReadOnly && <Button variant="secondary" onClick={onEditSchedule}>Change date & time</Button>}<Button variant="secondary" onClick={() => onNavigate('overview')}>Exit meeting</Button></div>
    </div>
    ...
    <div className="meeting-workspace">
      ...
      <section className="meeting-stage card-surface">
        ...
        <div className="meeting-stage-body"><MeetingSectionContentV2 section={section} workspace={workspace} team={team} rocks={rocks} todos={todos} issues={shortIssues} metrics={metrics} scorecardResults={scorecardResults} headlines={headlines} meeting={meeting} recap={recap} setRecap={setRecap} rating={rating} setRating={setRating} readOnly={meetingReadOnly} onUpdateRock={onUpdateRock} onUpdateTodo={onUpdateTodo} onOpenIssue={onOpenIssue} onOpenTodo={onOpenTodo} onStartIssue={onStartIssue} onSolveIssue={onSolveIssue} onCreateIssueFromScorecard={onCreateIssueFromScorecard} onCreateIssueFromRock={onCreateIssueFromRock} onSaveSectionNote={onSaveSectionNote} onReorderIssues={onReorderIssues} onNavigate={onNavigate} /></div>
        <div className="meeting-stage-footer"><button className="footer-nav-button" onClick={() => agenda[Math.max(0, currentIndex - 1)] && onSelectSection(agenda[Math.max(0, currentIndex - 1)].id)} disabled={currentIndex === 0}>← Previous</button><div className="footer-progress"><ProgressBar value={(currentIndex + 1) / Math.max(1, agenda.length) * 100} /><span>{currentIndex + 1} of {agenda.length}</span></div>{section === 'conclude' && !closed ? <Button onClick={() => onClose(recap, rating)} disabled={meetingReadOnly}>Close meeting ✓</Button> : <button className="footer-nav-button footer-nav-next" onClick={next} disabled={currentIndex === agenda.length - 1}>Next section →</button>}</div>
      </section>
    </div>
  </>;
}
...
function MeetingSectionContentV2({ section, workspace, team, rocks, todos, issues, metrics, scorecardResults, headlines, meeting, recap, setRecap, rating, setRating, readOnly, onUpdateRock, onUpdateTodo, onOpenIssue, onOpenTodo, onStartIssue, onSolveIssue, onCreateIssueFromScorecard, onCreateIssueFromRock, onSaveSectionNote, onReorderIssues, onNavigate }: MeetingSectionContentV2Props) {
...
function TodoEditModal({ workspace, todo, readOnly, onClose, onSubmit }: { workspace: Workspace; todo: Todo; readOnly: boolean; onClose: () => void; onSubmit: (input: Partial<Pick<Todo, 'title' | 'notes' | 'ownerId' | 'dueDate' | 'status'>>) => void }) {
  const submit = (event: FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    if (readOnly) return;
    const form = new FormData(event.currentTarget);
    onSubmit({ title: String(form.get('title') ?? '').trim(), notes: String(form.get('notes') ?? '').trim(), ownerId: String(form.get('ownerId') ?? ''), dueDate: String(form.get('dueDate') ?? ''), status: String(form.get('status') ?? todo.status) as TodoStatus });
  };
  return <ModalShell title="Open To-Do" description={readOnly ? 'This To-Do is shown for reference. Closed or read-only meeting records cannot be edited.' : 'Add working notes, adjust the owner or due date, and keep the commitment visible to the team.'} onClose={onClose}><form className="modal-form" onSubmit={submit}><fieldset className="modal-fieldset" disabled={readOnly}><label>Commitment<input name="title" defaultValue={todo.title} autoFocus required /></label><label>Notes<textarea name="notes" defaultValue={todo.notes} rows={6} placeholder="Add context, progress, or the next step." /></label><label>Owner<UserOptions workspace={workspace} name="ownerId" defaultValue={todo.ownerId} /></label><div className="form-grid"><label>Due date<input name="dueDate" type="date" defaultValue={todo.dueDate} required /></label><label>Status<select name="status" defaultValue={todo.status}><option value="open">Open</option><option value="done">Done</option><option value="not-done">Not done</option></select></label></div><div className="todo-rollover-callout"><strong>Moved forward {todo.carryForwardCount} times</strong><span>{todo.flagged ? 'This To-Do has become an Issue for IDS.' : 'Changing this due date to a later date records an automatic rollover. Completed To-Dos never accrue rollovers.'}</span></div></fieldset><div className="modal-actions"><Button variant="secondary" onClick={onClose}>Close</Button>{!readOnly && <Button type="submit">Save To-Do →</Button>}</div></form></ModalShell>;
}
...
function MeetingScheduleModal({ team, meeting, onClose, onSubmit }: { team: Team; meeting: Workspace['meetings'][number]; onClose: () => void; onSubmit: (input: { scheduledDate: string; scheduledTime: string }) => void }) {
  return <ModalShell title="Update the current meeting" description={`Change this ${cadenceLabel(team.meetingCadence).toLowerCase()} L10 occurrence without changing the team’s recurring cadence.`} onClose={onClose}>
